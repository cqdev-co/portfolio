    ...
    def add_trade(self, trade: SpreadTrade) -> bool:
        ...
        for t in self.trades:
            if (
                ...
                and t.long_strike == trade.long_strike
            ):
                return False
...
class MarketDataFetcher:
    ...
    def __init__(self):
        ...
        if not self.proxy_url:
            print(
                "WARNING: YAHOO_PROXY_URL not set. "
                "Entry conditions will not be fetched."
            )

    def fetch_entry_conditions(self, ticker: str, entry_date: str) -> dict:
        ...
        try:
            ...
            resp = requests.get(f"{self.proxy_url}/ticker/{ticker}", timeout=30)
            ...
            if price and result["entry_52w_high"]:
                result["entry_pct_from_52w_high"] = round(
                    (result["entry_52w_high"] - price) / result["entry_52w_high"] * 100,
                    2,
                )
            if price and result["entry_52w_low"]:
                result["entry_pct_from_52w_low"] = round(
                    (price - result["entry_52w_low"]) / result["entry_52w_low"] * 100, 2
                )
            if price and result["entry_50dma"]:
                result["entry_pct_from_50dma"] = round(
                    (price - result["entry_50dma"]) / result["entry_50dma"] * 100, 2
                )
            if price and result["entry_200dma"]:
                result["entry_pct_from_200dma"] = round(
                    (price - result["entry_200dma"]) / result["entry_200dma"] * 100, 2
                )
            ...
            if len(closes) >= 14:
                ...
                if volumes and len(volumes) >= 20:
                    ...
                    if avg_vol > 0:
                        result["entry_volume_ratio"] = round(volumes[-1] / avg_vol, 2)
            ...
            if len(closes) >= 20:
                ...
                if price and result["entry_50dma"] and result["entry_200dma"]:
                    ...
                    ma50_above_200 = result["entry_50dma"] > result["entry_200dma"]
...
class QuantAnalyzer:
    ...
    def basic_stats(self) -> dict:
        ...
        margin = (
            z
            * math.sqrt((win_rate * (1 - win_rate) + z**2 / (4 * self.n)) / self.n)
            / (1 + z**2 / self.n)
        )
    ...
    def regression(self) -> dict:
        ...
        X = [[1, t.entry_analyst_pct, t.entry_iv, t.days_held] for t in complete]

        # Solve normal equations: (X'X)^-1 X'y
        # Simplified 4x4 Gauss-Jordan
        XtX = [
            [sum(X[k][i] * X[k][j] for k in range(n)) for j in range(4)]
            for i in range(4)
        ]
        ...
        try:
            ...
            # Calculate R-squared
            y_pred = [sum(betas[j] * X[i][j] for j in range(4)) for i in range(n)]
            y_mean = sum(y) / n
    ...
    def _solve_linear(self, A: list, b: list) -> list:
        ...
        return [M[i][n] / M[i][i] if M[i][i] != 0 else 0 for i in range(n)]
    ...
    def print_report(self):
        ...
        print(f"   Required for significance: n ≥ {req_n} (to detect r ≥ 0.5)")
    ...
    for t in trades:
        ...
        if db.add_trade(trade):
            ...
            status = (
                "WIN"
                if t["is_winner"]
                else ("LOSS" if t["is_winner"] is False else "OPEN")
            )
...
def cmd_analyze(args):
    ...
    if not closed:
        ...
        print("Import trades first: python spread_quant_analysis.py import pf.csv")
        return
    ...
    def profile(trades, label):
        ...
        analysts = [t.entry_analyst_pct for t in trades if t.entry_analyst_pct]
    ...
    # Analyst threshold
    high_analyst = [
        t for t in closed if t.entry_analyst_pct and t.entry_analyst_pct >= 80
    ]
    low_analyst = [
        t for t in closed if t.entry_analyst_pct and t.entry_analyst_pct < 80
    ]
    ...
    if high_analyst:
        ...
        print(
            f"   Analyst ≥80%: {len(high_analyst)} trades, "
    ...
    if low_analyst:
        ...
        print(
            f"   Analyst <80%: {len(low_analyst)} trades, "
            f"WR={wr * 100:.0f}%, Avg={ret:+.1f}%"
        )
    ...
    if low_iv:
        ...
        print(f"   IV ≤50%: {len(low_iv)} trades, WR={wr * 100:.0f}%, Avg={ret:+.1f}%")
    if high_iv:
        wr = sum(1 for t in high_iv if t.pnl > 0) / len(high_iv)
        ret = avg([t.return_pct for t in high_iv])
        print(f"   IV >50%: {len(high_iv)} trades, WR={wr * 100:.0f}%, Avg={ret:+.1f}%")
    ...
    for ticker in sorted(by_ticker.keys()):
        ...
        print(
            f"   {status} {ticker}: {len(trades)} trades, "
...
def cmd_show(args):
    ...
    for t in sorted(db.trades, key=lambda x: x.entry_date):
        ...
        print(
            f"{t.ticker:<8} {t.entry_date:<12} {exit_d:<12} "
...
def cmd_fetch(args):
    ...
    if args.ticker:
        ...
        trades = [t for t in db.trades if t.entry_price is None or t.entry_beta is None]
...
def cmd_template(args):
    ...
    fields = [
        ...
        "entry_date",
        "entry_price",
        "entry_pct_from_52w_high",
        "entry_pct_from_50dma",
        "entry_pe",
        "entry_beta",
        "entry_market_cap",
        "entry_rsi",
    ...
    print("Fill in the missing values and run:")
...
def cmd_coverage(args):
    ...
    categories = {
        ...
        "Sentiment": [
            ...
            "entry_short_pct",
            "entry_put_call_ratio",
            "entry_institutional_pct",
        ],
        "Volatility": ["entry_iv", "entry_iv_rank", "entry_hv_20", "entry_iv_hv_ratio"],
        "Timing": [
            "entry_day_of_week",
            "entry_days_to_earnings",
    ...
    for cat, fields in categories.items():
        print(f"\n   {cat}:")
        for field in fields:
            count = sum(1 for t in closed if getattr(t, field, None) is not None)
            pct = count / len(closed) * 100
            bar = "█" * int(pct / 5) + "░" * (20 - int(pct / 5))
            status = "✓" if pct >= 50 else "○"
            print(
                f"   {status} {field:<28} {bar} {count:>3}/{len(closed)} ({pct:.0f}%)"
            )
    ...
    filled_cells = sum(
        1 for t in closed for f in all_fields if getattr(t, f, None) is not None
    )
    overall = filled_cells / total_cells * 100

    print("\n" + "-" * 70)
    print(f"   Overall Coverage: {filled_cells}/{total_cells} cells ({overall:.1f}%)")
    print(
        f"   Factors with >50% coverage: "
...
def cmd_open(args):
    ...
    if winners and losers:
        w_analyst = avg([t.entry_analyst_pct for t in winners if t.entry_analyst_pct])
        l_analyst = avg([t.entry_analyst_pct for t in losers if t.entry_analyst_pct])
        ...
        if w_analyst and l_analyst:
            print(
                f"   • Analyst ≥ {int(w_analyst)}% "
                f"(winners avg {w_analyst:.0f}% vs {l_analyst:.0f}%)"
            )
        if w_iv and l_iv:
            print(f"   • IV ≤ {int(l_iv)}% (winners avg {w_iv:.0f}% vs {l_iv:.0f}%)")
        if w_days:
            print(f"   • Hold ≥ {int(w_days)} days (winners avg {w_days:.0f} days)")
    ...
    for t in open_trades:
        ...
        print(
            f"   Entry: {t.entry_date} | Days held: {days_held} | "
...
def cmd_batch_update(args):
    ...
    field_map = {
        "rsi": "entry_rsi",
        "rsi_5d": "entry_rsi_5d",
        "buffer": "entry_buffer_pct",
        "analyst": "entry_analyst_pct",
        "iv": "entry_iv",
        "trend": "entry_trend_pct",
        "pe": "entry_pe",
        "beta": "entry_beta",
        ...
        "bb_position": "entry_bb_position",
        "trend_5d": "entry_trend_5d",
        "trend_20d": "entry_trend_20d",
        "short_ratio": "entry_short_ratio",
        "put_call": "entry_put_call_ratio",
        "iv_rank": "entry_iv_rank",
        "hv_20": "entry_hv_20",
            ...
            for t in db.trades:
                if t.ticker == ticker and t.entry_date == entry_date:
                    ...
                    if fields_set > 0:
                        t.updated_at = datetime.now().isoformat()
                        updated += 1
                        print(f"  Updated {ticker} {entry_date} ({fields_set} fields)")
                    break
...
def main():
    parser = argparse.ArgumentParser(description="Spread Quantitative Analysis System")
    ...
    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Run quantitative analysis")
    analyze_parser.set_defaults(func=cmd_analyze)

    # Show command
    show_parser = subparsers.add_parser("show", help="Show trade database")
    show_parser.set_defaults(func=cmd_show)

    # Open positions command
    open_parser = subparsers.add_parser("open", help="Analyze open positions")
    ...
    # Fetch command
    fetch_parser = subparsers.add_parser("fetch", help="Fetch data from Yahoo proxy")
    ...
    batch_parser = subparsers.add_parser("batch-update", help="Batch update from CSV")
    batch_parser.add_argument("csv_file", help="CSV with ticker,entry_date,rsi,...")
