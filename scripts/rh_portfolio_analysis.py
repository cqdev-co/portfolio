...
class RobinhoodCSVParser:
    ...
    def _merge_multiline_rows(self, rows: list[dict]) -> list[dict]:
        ...
        while i < len(rows):
            ...
            if (
                i + 1 < len(rows)
                and rows[i + 1].get("Activity Date") == ""
                and "CUSIP" in str(rows[i + 1].get("Description", ""))
            ):
                # This is a multi-line stock description, merge
                row["Description"] = (
                    f"{row.get('Description', '')} {rows[i + 1].get('Description', '')}"
                )
                i += 2
            else:
                i += 1
    ...
    def _process_row(self, row: dict) -> None:
        ...
        _description = row.get("Description", "").strip()  # noqa: F841
        ...
        # Other income
            ...
            self.other_income.append(
                OtherIncome(date=date, type=trans_code, ticker=ticker, amount=amount)
            )
...
class SpreadAnalyzer:
    ...
    def __init__(
        ...
        self.option_trades = sorted(option_trades, key=lambda x: x.date)
    ...
    def _process_spread_group(self, trades: list[OptionTrade]) -> None:
        ...
        for long_leg, short_leg in open_pairs:
            ...
            for close_long, close_short in close_pairs:
                ...
                if (
                    close_short.strike == long_leg.strike
                    and close_long.strike == short_leg.strike
                ):
                    spread.exit_credit = close_long.amount + close_short.amount
    ...
    def _find_simultaneous_pairs(
        ...
        for i, t1 in enumerate(sorted_trades):
            ...
            for j, t2 in enumerate(sorted_trades[i + 1 :], i + 1):
                ...
                if (
                    t1.date == t2.date
                    and t1.strike != t2.strike
                    and t1.is_long != t2.is_long
                ):
    ...
    def _calculate_metrics(self) -> AnalysisResults:
        ...
        closing_option_trades = [
            t for t in self.option_trades if t.action in ("BTC", "STC")
        ]
        _opening_option_trades = [  # noqa: F841
            t for t in self.option_trades if t.action in ("BTO", "STO")
        ]

        # Realized option P&L = sum of all closing transactions
        # (STC gives positive cash, BTC gives negative cash)
        _option_close_amounts = sum(t.amount for t in closing_option_trades)  # noqa: F841
        ...
        if losses:
            results.profit_factor = abs(results.total_gains) / abs(results.total_losses)
        ...
        results.capital_at_risk = sum(abs(s.entry_debit) for s in open_pos)
    ...
    def _spread_to_dict(self, spread: SpreadTrade) -> dict:
        ...
        return {
            ...
            "entry_date": (
                spread.entry_date.strftime("%Y-%m-%d") if spread.entry_date else None
            ),
            "exit_date": (
                spread.exit_date.strftime("%Y-%m-%d") if spread.exit_date else None
            ),
    ...
    def _generate_warnings(self, results: AnalysisResults) -> list[str]:
        ...
        if results.avg_debit_paid > 0 and results.avg_spread_width > 0:
            risk_reward = results.avg_debit_paid / (
        ...
        for ticker, perf in results.ticker_performance.items():
            if perf["trades"] >= 2 and perf["total_pnl"] < -50:
                ...
                pnl_str = f"-${abs(pnl_val):.2f}" if pnl_val < 0 else f"+${pnl_val:.2f}"
                warnings.append(
    ...
    def _generate_insights(self, results: AnalysisResults) -> list[str]:
        ...
        if results.ticker_performance:
            best = max(
                results.ticker_performance.items(), key=lambda x: x[1]["total_pnl"]
            )
            if best[1]["total_pnl"] > 0:
                insights.append(
                    f"💰 Best performer: {best[0]} (+${best[1]['total_pnl']:.2f})"
                )

        # Risk management
        if results.total_spreads > 0:
            insights.append(
                f"✅ Defined risk: Max loss per trade ~${results.avg_debit_paid:.0f}"
            )

        # Profit factor
        if results.profit_factor >= 1.0:
            insights.append(
                f"📈 Profit factor: {results.profit_factor:.2f} (gains/losses ratio)"
            )

        # Consistency
        if results.avg_days_held < 14:
            insights.append(
                f"⏱️ Active management: Avg hold time {results.avg_days_held:.0f} days"
            )
...
def print_report(results: AnalysisResults, verbose: bool = False) -> None:
    ...
    print("     (Spreads opened & closed in this period + income)")
    print()

    # Breakdown of verifiable
    spread_str = (
        f"+${results.spread_pnl:.2f}"
        if results.spread_pnl >= 0
        else f"-${abs(results.spread_pnl):.2f}"
    )
    income_str = (
        f"+${results.other_income_total:.2f}"
    ...
    if results.unmatched_closes:
        unmatched_str = (
            f"+${results.unmatched_close_value:.2f}"
    ...
    stock_str = (
        f"+${results.stock_pnl:.2f}"
        if results.stock_pnl >= 0
        else f"-${abs(results.stock_pnl):.2f}"
    )
    ...
    loss_str = (
        f"-${abs(results.total_losses):.2f}"
        if results.total_losses < 0
        else f"${results.total_losses:.2f}"
    )
    avg_loser_str = (
        f"-${abs(results.avg_loser):.2f}"
        if results.avg_loser < 0
        else f"${results.avg_loser:.2f}"
    )
    largest_loser_str = (
        f"-${abs(results.largest_loser):.2f}"
        if results.largest_loser < 0
        else f"${results.largest_loser:.2f}"
    )
    ...
    if results.avg_spread_width > 0:
        ...
        if max_profit > 0:
            ...
            print(
                f"  Required Win Rate: {(risk_reward / (1 + risk_reward)) * 100:.0f}%"
            )
    ...
    if verbose and results.other_income:
        ...
        for inc in results.other_income:
            ...
            print(f"    {inc['date']} | {inc['type']:6} | {amt_str}")
    ...
    problem_tickers = [
        t
        for t, p in results.ticker_performance.items()
...
def main():
    ...
    parser.add_argument("csv_file", help="Path to Robinhood CSV export")
    parser.add_argument(
        "--output",
        "-o",
        choices=["text", "json"],
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed trade history"
    )
