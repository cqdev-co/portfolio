...
def analyze_trade(ticker: TickerData, pnl: float, status: str = "closed") -> dict:
    ...
    if ticker.forward_pe:
        if ticker.forward_pe > 100:
            warnings.append(f"⚠️ EXTREME valuation: Fwd P/E {ticker.forward_pe:.1f}")
        elif ticker.forward_pe > 50:
            warnings.append(f"⚠️ High valuation: Fwd P/E {ticker.forward_pe:.1f}")
    ...
    if ticker.atm_iv:
        if ticker.atm_iv > 60:
            ...
            warnings.append(
                f"⚠️ Elevated IV ({ticker.atm_iv:.0f}%): Options moderately expensive"
            )
        elif ticker.atm_iv < 25:
            positives.append(
                f"✅ Low IV ({ticker.atm_iv:.0f}%): Good value for debit spreads"
            )
    ...
    if ticker.bullish_pct is not None:
        if ticker.bullish_pct < 50:
            warnings.append(f"⚠️ Bearish analysts: Only {ticker.bullish_pct}% bullish")
        elif ticker.bullish_pct < 70:
            warnings.append(f"⚠️ Mixed analyst sentiment: {ticker.bullish_pct}% bullish")
    ...
    if ticker.pct_from_50dma is not None:
        if ticker.pct_from_50dma < -5:
            warnings.append(
                f"⚠️ Below 50DMA: {ticker.pct_from_50dma:.1f}% (weak near-term trend)"
            )
        elif ticker.pct_from_50dma > 5:
            positives.append(
                f"✅ Above 50DMA: +{ticker.pct_from_50dma:.1f}% (strong momentum)"
            )
    ...
    if status == "open" and ticker.earnings_days is not None:
        if ticker.earnings_days <= 30:
            warnings.append(
                f"🚨 EARNINGS in {ticker.earnings_days} days! High binary risk"
            )
...
def main():
    ...
    avg_loser_warnings = (
        sum(r["warning_count"] for r in losers) / len(losers) if losers else 0
    )
    avg_winner_warnings = (
        sum(r["warning_count"] for r in winners) / len(winners) if winners else 0
    )
    ...
    # Find patterns
    high_iv_losers = [r for r in losers if r["data"].get("iv") and r["data"]["iv"] > 50]
    if high_iv_losers:
    ...
    below_ma_losers = [
        r for r in losers if r["data"].get("pct_50dma") and r["data"]["pct_50dma"] < -5
    ]
    if below_ma_losers:
    ...
    if earnings_risk:
        for r in earnings_risk:
            print(
                f"4. ⚠️ {r['symbol']}: Consider closing before earnings "
