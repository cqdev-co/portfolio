...
export function encodeTickerToTOON(data: TickerData): string {
  ...
  if (data.iv?.currentIV !== undefined || data.hv20 !== undefined) {
    const iv = data.iv?.currentIV 
      ? Math.round(data.iv.currentIV) 
      : null;
  ...
  if (data.earningsDays !== undefined && data.earningsDays !== null) {
    tickerData.earnings = data.earningsDays > 0 
      ? `${data.earningsDays}d` 
      : 'passed';
  }
  ...
  if (data.targetPrices?.mean !== undefined) {
    tickerData.targets = {
      ...
      upside: data.targetPrices.upside !== undefined
        ? `${data.targetPrices.upside > 0 ? '+' : ''}${
            data.targetPrices.upside.toFixed(0)
          }%`
        : null,
    };
  }
  ...
  if (data.performance) {
    ...
    if (data.performance.day5 !== undefined) {
      perf.d5 = `${data.performance.day5 > 0 ? '+' : ''}${
        data.performance.day5.toFixed(1)
      }%`;
    }
    if (data.performance.month1 !== undefined) {
      perf.m1 = `${data.performance.month1 > 0 ? '+' : ''}${
        data.performance.month1.toFixed(1)
      }%`;
    }
    if (data.performance.ytd !== undefined) {
      perf.ytd = `${data.performance.ytd > 0 ? '+' : ''}${
        data.performance.ytd.toFixed(1)
      }%`;
    }
  ...
  if (data.shortInterest?.shortPct !== undefined && 
  ...
  if (data.pfv) {
    tickerData.pfv = {
      ...
      dev: `${data.pfv.deviationPercent > 0 ? '+' : ''}${
        data.pfv.deviationPercent.toFixed(1)
      }%`,
    };
  }
...
export function encodeMarketRegimeToTOON(regime: MarketRegime): string {
  const data = {
    ...
    vix: {
      ...
      change: `${regime.vix.changePct >= 0 ? '+' : ''}${
        regime.vix.changePct
      }%`,
    },
    spy: {
...
export function encodeTickerTableToTOON(tickers: TickerData[]): string {
  ...
  const rows = tickers.map(t => ({
    ...
    cushion: t.spread?.cushion 
      ? Number(t.spread.cushion.toFixed(1)) 
      : null,
    grade: t.grade?.grade ?? null,
