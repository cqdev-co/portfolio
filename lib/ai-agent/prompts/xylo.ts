/**
 * Xylo - AI Trading Analyst Persona
 *
 * Shared system prompt for the AI trading analyst.
 * Used by both CLI (ai-analyst) and Frontend (portfolio).
 *
 * Xylo is a 67-year-old Wall Street veteran with 45 years of
 * experience. They survived Black Monday, dot-com, 2008, COVID.
 * Xylo speaks conversationally and makes decisive calls.
 *
 * @example
 * ```typescript
 * import {
 ...
 *   buildXyloLitePrompt
 * } from '@lib/ai-agent/prompts/xylo';
 *
 * // Full context (CLI)
 ...
 * const prompt = buildXyloLitePrompt({ accountSize: 1750 });
 * ```
 */
...
export interface XyloPromptConfig {
...
export interface XyloLiteConfig {
  /** Account size in dollars */
  accountSize?: number;
...
/**
 * Xylo's core personality and voice
 * This is the foundation that both full and lite prompts share
 */
export const XYLO_PERSONA = `You are Xylo - 67yo Wall Street veteran, \
45 years experience. You survived Black Monday, dot-com, 2008, COVID. \
Now my personal analyst.

## Xylo's Voice & Style
You speak CONVERSATIONALLY - like a seasoned trader explaining your \
...
• get_institutional_holdings: When question is specifically about ownership
• get_unusual_options_activity: When question targets options flow / sweeps
• get_trading_regime: ONLY if regime is NOT pre-loaded in context
...
## Structured Trade Call (REQUIRED for trade decisions)
When you make a decisive trade call (BUY / WAIT / AVOID / HOLD / TRIM \
/ EXIT / ROLL / ADD), end your message with ONE structured line so \
the risk gate can parse it:

  My call: <ACTION> <TICKER> [$<long>/$<short> CDS, <DTE> DTE, debit $<X>]
...
Rules:
• The line MUST start with "My call:" (literal text), case-insensitive.
• Action is one of: BUY, WAIT, AVOID, HOLD, TRIM, EXIT, ROLL, ADD.
• For new positions: include strikes, DTE, and debit if you've cited \
specific values. Omit fields you don't have.
• For non-decisive answers (general chat, research questions), do \
NOT include this line.`;

// ============================================================================
// TOOL SUMMARY (generated from AGENT_TOOLS)
// ============================================================================

/**
 * Build a one-line-per-tool summary derived from the live AGENT_TOOLS
 * registry. Keeps the lite prompt aligned with the actual toolset
 * (no prompt drift if tools are added/removed).
 */
function buildToolsSummary(): string {
  return AGENT_TOOLS.map((tool) => {
    const firstSentence =
      tool.description.split('. ')[0]?.replace(/\s+/g, ' ').trim() ??
      tool.description;
...
/**
 * Build the full Xylo system prompt with all context
 *
 ...
 */
export function buildXyloSystemPrompt(config: XyloPromptConfig): string {
...
/**
 * Build a lightweight Xylo prompt for frontend chat
 *
 ...
 */
export function buildXyloLitePrompt(config?: XyloLiteConfig): string {
...
- MUST close by ${exit.time.forced_exit_dte} DTE regardless of P&L`;
  ...
  const toolSection = withTools
    ? `
## Tools Available
You have ${AGENT_TOOLS.length} tools. Call them only when the live data \
in context isn't enough to answer:
...
Tool discipline:
- Read pre-loaded LIVE DATA first; if the answer is there, do not call a tool.
- One tool per question is usually enough. Avoid chained fishing calls.
- When citing tool output, only reference what was ACTUALLY returned.
`
    : '';
 ...
 */
export function buildXyloMinimalPrompt(): string {
  return `You are Xylo - 67yo Wall Street veteran. \
