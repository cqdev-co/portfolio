...
def search_tickers(
    ...
    try:
        ...
        query_builder = (
            _get_ticker_client().client.table("tickers").select("*").or_(or_condition)
        )
...
def get_ticker_count(active_only: bool = True) -> int:
    ...
    """
    try:
        query = _get_ticker_client().client.table("tickers").select("id", count="exact")
...
def get_exchanges() -> List[str]:
    ...
    try:
        response = (
            ...
            .client.table("tickers")
            .select("exchange")
            .not_.is_("exchange", "null")
            .execute()
        )

        exchanges = list(
            set(item["exchange"] for item in response.data if item["exchange"])
        )
        exchanges.sort()
...
def get_countries() -> List[str]:
    ...
    try:
        ...
        countries = list(
            set(item["country"] for item in response.data if item["country"])
        )
        countries.sort()
...
def get_sectors() -> List[str]:
    ...
    try:
        ...
        sectors = list(set(item["sector"] for item in response.data if item["sector"]))
        sectors.sort()
