/**
 * Shared ticker management
 *
 * Fetches ticker lists from Supabase, Wikipedia, or fallback lists.
 * Used by all engine strategies.
 */
...
function loadSP500Cache(): SP500CacheData | null {
  try {
    if (fs.existsSync(SP500_CACHE_FILE)) {
      ...
      if (data.tickers && data.timestamp) return data as SP500CacheData;
    }
  } catch {
...
export async function fetchTickersFromDB(
  ...
  try {
    while (hasMore) {
      ...
      logger.debug(
        `Fetched ${data.length} tickers (offset: ${offset}, total: ${allTickers.length})`
      );
      offset += PAGE_SIZE;
      hasMore = data.length === PAGE_SIZE;
      if (options.limit && allTickers.length >= options.limit) hasMore = false;
    }
...
export async function fetchSP500Tickers(options?: {
  ...
  return limit ? SP500_FALLBACK.slice(0, limit) : SP500_FALLBACK;
}

export async function fetchAllStockTickers(limit?: number): Promise<string[]> {
  return fetchTickersFromDB({ tickerType: 'stock', limit });
}
