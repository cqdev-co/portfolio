...
-- 7. Views
CREATE OR REPLACE VIEW signals_today AS
SELECT
  s.*,
  CASE
    WHEN s.strategy = 'cds' THEN 'Call Debit Spread'
    WHEN s.strategy = 'pcs' THEN 'Put Credit Spread'
    WHEN s.strategy = 'penny' THEN 'Penny Stock'
    ELSE s.strategy
  END AS strategy_name
FROM signals s
WHERE s.signal_date = CURRENT_DATE
ORDER BY s.score_normalized DESC;

CREATE OR REPLACE VIEW signals_leaderboard AS
SELECT
  s.ticker,
  s.signal_date,
  s.strategy,
  ...
  s.price,
  s.regime,
  COUNT(*) OVER (PARTITION BY s.ticker, s.signal_date) AS strategy_count
FROM signals s
WHERE s.signal_date >= CURRENT_DATE - INTERVAL '7 days'
  AND s.grade IN ('S', 'A')
ORDER BY s.signal_date DESC, s.score_normalized DESC;
