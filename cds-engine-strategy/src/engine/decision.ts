...
export function evaluateEntry(input: DecisionEngineInput): EntryDecision {
  ...
  // If price is within 5% of MA200 and trending up, consider SCALE_IN
  const nearMA200Reclaim =
    belowMA200 &&
    input.ma200 &&
    (input.ma200 - input.currentPrice) / input.ma200 < 0.05;

  // Decision matrix - now handles missing spread data gracefully
  // v2.6.0: Added SCALE_IN for two-stage entry
  ...
  } else if (nearMA200Reclaim && confidence.total >= 60) {
    // v2.6.0: Near MA200 reclaim with decent confidence = SCALE_IN
    // Start with half position, add when MA200 is reclaimed
    action = 'scale_in';
    timeframe = '1-3_days';
    reasoning.push('Near MA200 reclaim — scale in with 50% position');
    reasoning.push('Add remaining 50% when price closes above MA200');
    warnings.push('⚠️ Price below MA200 — using two-stage entry');
  ...
  // v2.6.0: Added SCALE_IN guidance
  ...
  } else if (action === 'scale_in' && bestSpread) {
    // v2.6.0: Two-stage entry - start with half position
    const halfContracts = Math.max(
      1,
      Math.floor(positionSizing.maxContracts / 2)
    );
    entryGuidance.push(`STAGE 1 (Today): Enter with 50% position`);
    entryGuidance.push(
      `  Buy ${halfContracts} × $${bestSpread.longStrike}C / ` +
        `$${bestSpread.shortStrike}C spread`
    );
    entryGuidance.push(
      `  Cost: $${(halfContracts * bestSpread.netDebit * 100).toFixed(0)}`
    );
    entryGuidance.push(
      `STAGE 2: Add remaining 50% when price closes above MA200`
    );
    if (input.ma200) {
      entryGuidance.push(`  MA200 target: $${input.ma200.toFixed(2)}`);
    }
  } else if (action === 'scale_in' && !bestSpread) {
    entryGuidance.push(`STAGE 1: Enter with 50% position at current price`);
    entryGuidance.push(`STAGE 2: Add 50% when price reclaims MA200`);
    if (input.ma200) {
      entryGuidance.push(`  MA200 target: $${input.ma200.toFixed(2)}`);
    }
    entryGuidance.push(`Options: Target 6-10% ITM for deep ITM spread`);
 ...
 * v2.6.0: Added SCALE_IN formatting
 */
