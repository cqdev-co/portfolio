...
// ============================================================================
// OPTIONS CHAIN VIA PROXY
// ============================================================================

/**
 * Options chain response from proxy's /options-chain/:ticker endpoint
 */
export interface ProxyOptionsChainResponse {
  ...
  availableExpirations: string[]; // All available expirations as YYYY-MM-DD
  calls: Array<{
    strike: number;
    lastPrice: number;
    bid: number;
...
/**
 * Fetch full options chain via proxy (bypasses client-side rate limits)
 * This is the preferred method for spread scanning
 *
 * @param ticker - Stock symbol
 * @param targetDTE - Target days to expiration (finds closest available)
 */
export async function fetchOptionsChainViaProxy(
  ...
  if (!isProxyConfigured()) {
    logger.debug(`[Proxy] Options chain: proxy not configured`);
    return null;
  }
  ...
  try {
    ...
    // Fetch from proxy with target date
...
export async function fetchOptionsChainCached(
  ...
  if (cached) {
    stats.cacheHits++;
    logger.debug(`[Cache] Options chain HIT for ${ticker}`);
    return cached;
  }
  ...
  if (data) {
    setCache(cacheKey, data, CACHE_TTL.optionsChain);
  }
...
const CACHE_TTL = {
  ...
  optionsChain: 60_000, // 1 minute for options chain (prices change)
};
