...
function wrapText(text, maxLength = MAX_LINE_LENGTH) {
  ...
  if (
    ...
    // Removed text.includes('](') to allow link wrapping
      ...
      if (colonIndex > 0) {
        ...
        if (value.startsWith('"') && value.endsWith('"')) {
          ...
          return (
            field +
            ' >\n  ' +
            wrapPlainText(content, maxLength - 2)
              .split('\n')
              .join('\n  ')
          );
        }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.length > MAX_LINE_LENGTH) {
      ...
      if (wrappedLine !== line) {
        changes++;
        console.log(
          `  Line ${i + 1}: ${line.length} → ${Math.max(...wrappedLine.split('\n').map((l) => l.length))} chars`
        );
      }
...
function main() {
  console.log(
    '🔧 Line Length Fixer - Fixing lines longer than 75 characters\n'
  );
