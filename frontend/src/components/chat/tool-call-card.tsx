  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
        ...
        {data.ma200 && (
          <DataRow label="MA200" value={`$${data.ma200.toFixed(0)}`} />
        )}
        {data.marketCap && (
          <DataRow label="Market Cap" value={formatMarketCap(data.marketCap)} />
        )}
        {data.peRatio && (
          <DataRow label="P/E" value={data.peRatio.toFixed(1)} />
        )}
        {data.analystRatings && (
          <DataRow
            label="Sentiment"
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            ...
            {data.earnings?.streak && (
              <DataRow
                label="Streak"
                value={
                  data.earnings.streak > 0
                    ? `${data.earnings.streak} beats`
                    : `${Math.abs(data.earnings.streak)} misses`
                }
                className={
                  data.earnings.streak > 0 ? 'text-emerald-600' : 'text-red-500'
                }
              />
            )}
      ...
      {data.iv && (
        <div className="pt-2 border-t border-border/50">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            ...
            {data.iv.hv20 && (
              <DataRow label="HV20" value={`${data.iv.hv20.toFixed(0)}%`} />
            )}
            {data.iv.premium && (
              <DataRow
                label="Options"
      ...
      {data.grade && (
        <div className="pt-2 border-t border-border/50">
          <div className="flex items-center justify-between">
            ...
            >
              {data.grade.grade} ({data.grade.score}/100) -{' '}
              {data.grade.recommendation}
            </span>
          </div>
        </div>
      )}
