...
export default function UnusualOptionsScanner() {
  ...
  const [selectedTicker, setSelectedTicker] =
    useState<GroupedTickerSignals | null>(null);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<
    'overview' | 'strikes' | 'expiries' | 'timeline' | 'all'
  >('overview');
  ...
  const calculateStatsFromSignals = (
    ...
    const average_score =
      signals.reduce((sum, s) => sum + (s.overall_score || 0), 0) /
      signals.length;
  ...
  useEffect(() => {
    const unsubscribe = subscribeToUnusualOptionsUpdates((payload) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
        ...
        if (e.key === 'ArrowUp') {
          nextIndex =
            currentIndex > 0 ? currentIndex - 1 : groupedTickers.length - 1;
        } else {
          nextIndex =
            currentIndex < groupedTickers.length - 1 ? currentIndex + 1 : 0;
        }
    ...
    <div className="flex flex-col gap-8">
      ...
      <header>
        ...
        <div className="text-compact text-muted-foreground space-y-2">
          <p>
            The <strong>unusual options scanner</strong> identifies suspicious
            options activity that might indicate insider information or smart
            money positioning. This professional <em>trading tool</em> analyzes{' '}
            <strong>premium flow</strong>, <strong>volume anomalies</strong>,
            and <strong>open interest spikes</strong> to detect high-conviction
            signals.
          </p>
          <p>
            Deploy this <strong>insider detection system</strong> to spot large,
            concentrated bets before major moves; these signals historically
            precede significant price movements. Perfect for{' '}
            <em>institutional traders</em> and <em>sophisticated investors</em>{' '}
            seeking edge in options markets.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              Active Insider Activity Detection
            </span>
          </div>
          <span
            className="text-xs text-muted-foreground"
            title="Options flow indicators"
          >
            High Flow • Premium Tracking
          </span>
        </div>
      ...
      >
        <h2 id="search-section-title" className="sr-only">
          Search and Filter Options Signals
        </h2>
              ...
              <span className="ml-1 text-xs bg-primary text-primary-foreground rounded-full h-4 w-4 flex items-center justify-center">
                {
                  Object.keys(filters).filter(
                    (k) =>
                      filters[k as keyof UnusualOptionsFilters] !== undefined
                  ).length
                }
              </span>
            )}
      ...
      <section aria-labelledby="signals-table-title">
        <h2 id="signals-table-title" className="sr-only">
          Live Unusual Options Signals
        </h2>
        <div
          className="rounded-md border"
          role="region"
          aria-label="Options signals data table"
        >
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {groupedTickers.length === 0 ? (
                ...
                groupedTickers.slice(0, 100).map((tickerGroup) => {
                  ...
                  const isSelected =
                    selectedTicker?.ticker === tickerGroup.ticker;
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge variant="secondary" className="text-xs">
                          {tickerGroup.signalCount} signal
                          {tickerGroup.signalCount !== 1 ? 's' : ''}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  );
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const nextIndex =
                            currentIndex < groupedTickers.length - 1
                              ? currentIndex + 1
                              : 0;
                          const nextTicker = groupedTickers[nextIndex];
                          if (nextTicker) {
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="text-[10px] text-muted-foreground space-y-1 bg-muted/20 p-2.5 rounded-lg border border-border/30">
                          <div className="flex items-center justify-between">
                            ...
                            <span>
                              {toLocaleStringEST(summary.dateRange.earliest)}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="font-medium">Latest:</span>
                            <span>
                              {toLocaleStringEST(summary.dateRange.latest)}
                            </span>
                          </div>
                        </div>
                      </div>
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] text-muted-foreground/60">
                                      {dateGroup.callCount}C /{' '}
                                      {dateGroup.putCount}P
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] text-muted-foreground/60">
                                      {dateGroup.signalCount} signal
                                      {dateGroup.signalCount > 1 ? 's' : ''}
                                    </span>
                                  </div>
                                </div>
                              </div>
                                ...
                                {dateGroup.signals.map((signal) => (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center justify-between text-[10px] text-muted-foreground/70">
                                      ...
                                      <span>
                                        Score:{' '}
                                        {(signal.overall_score * 100).toFixed(
                                          0
                                        )}
                                        %
                                      </span>
                                        ...
                                        {signal.is_likely_spread &&
                                          signal.spread_confidence && (
                                            <span
                                              ...
                                              title={
                                                signal.spread_detection_reason ||
                                                undefined
                                              }
                                            >
                                              ⚠️{' '}
                        ...
                        {selectedTicker.signals.slice(0, 20).map((signal) => (
                          ...
                          >
                              ...
                              <div className="flex flex-wrap gap-1">
                                ...
                                {signal.is_likely_spread &&
                                  signal.spread_confidence && (
                                    ...
                                    >
                                      ⚠️ {formatSpreadType(signal.spread_type)}{' '}
                                      (
                                      {formatSpreadConfidence(
                                        signal.spread_confidence
                                      )}
                            ...
                            <div className="text-[10px] text-muted-foreground font-medium pt-0.5 border-t border-border/30">
                              {toLocaleStringEST(signal.detection_timestamp)}{' '}
                              EST
                            </div>
                          </div>
                        ))}
                        {selectedTicker.signalCount > 20 && (
                          <div className="text-xs text-center text-muted-foreground p-3 border border-border/50 rounded-lg bg-muted/20">
                            <p className="font-medium">
                              Showing top 20 of {selectedTicker.signalCount}{' '}
                              signals
                            </p>
                            <p className="text-[10px] mt-1">
                              Use other tabs to explore all signals grouped by
                              strike, expiry, or timeline
                            </p>
                          </div>
                        )}
                      </div>
