...
import { formatDate, createMetadata, sortPostsByDate, filterPostsByTag, getAllTags } from "@/lib/utils";
...
async function BlogPageContent({ searchParams }: { searchParams: Promise<{ tag?: string }> }) {
  ...
  const filteredPosts = selectedTag ? filterPostsByTag(sortedPosts, selectedTag) : sortedPosts;
  ...
  return (
    <section className="max-w-3xl mx-auto">
      ...
      {filteredPosts.length === 0 && selectedTag && (
        <p className="text-muted-foreground">No posts found with the tag &ldquo;{selectedTag}&rdquo;. Try a different tag or view all posts.</p>
      )}
      {posts.length === 0 && (
