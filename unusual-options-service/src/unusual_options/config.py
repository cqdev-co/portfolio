...
def load_config() -> dict[str, Any]:
    ...
    config = {
        ...
        "VOLUME_MULTIPLIER_THRESHOLD": float(
            os.getenv("VOLUME_MULTIPLIER_THRESHOLD", "3.0")
        ),
        ...
        "AGGRESSIVE_ORDER_THRESHOLD": float(
            os.getenv("AGGRESSIVE_ORDER_THRESHOLD", "0.70")
        ),
        # Filtering
        ...
        "ENABLE_MEME_STOCK_FILTERING": os.getenv(
            "ENABLE_MEME_STOCK_FILTERING", "true"
        ).lower()
        == "true",
        # DTE Filtering (based on performance analysis)
        # Analysis showed: Short DTE (≤10d) = 27% win rate, Mid DTE (11-21d) = 60% win rate
        "MIN_DTE_ALL_TICKERS": int(
            os.getenv("MIN_DTE_ALL_TICKERS", "10")
        ),  # Minimum DTE for all tickers
        "MIN_DTE_HIGH_0DTE_TICKERS": int(
            os.getenv("MIN_DTE_HIGH_0DTE_TICKERS", "14")
        ),  # Stricter for TSLA, SPY, etc.
        "MIN_DTE_STANDARD": int(
            os.getenv("MIN_DTE_STANDARD", "10")
        ),  # Standard DTE filter
        ...
        "EXCLUDE_PUT_SIGNALS": os.getenv("EXCLUDE_PUT_SIGNALS", "false").lower()
        == "true",
        ...
        "MAX_SIGNALS_PER_TICKER": int(
            os.getenv("MAX_SIGNALS_PER_TICKER", "3")
        ),  # Reduced from 5
        # Alerts
        "DISCORD_WEBHOOK_URL": os.getenv(
            "DISCORD_UOS_WEBHOOK_URL", os.getenv("DISCORD_WEBHOOK_URL", "")
        ),
