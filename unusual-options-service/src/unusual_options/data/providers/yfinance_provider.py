...
class YFinanceProvider(DataProvider):
    ...
    def _handle_rate_limit_error(self, error_msg: str) -> None:
        ...
        logger.warning(f"Rate limit detected, backing off for {delay:.1f} seconds (attempt {self.consecutive_errors})")
    
    def _reset_error_count(self) -> None:
        """Reset consecutive error count on successful request."""
        if self.consecutive_errors > 0:
            logger.debug(f"Request successful, resetting error count from {self.consecutive_errors}")
            self.consecutive_errors = 0
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if any(phrase in error_msg for phrase in [
                    ...
                    'quota exceeded', 'requests per second'
                ]):
                ...
                # Check for other recoverable errors
                    ...
                    if attempt < self.max_retries:
                        ...
                        logger.warning(f"Recoverable error, retrying in {wait_time}s: {e}")
                        await asyncio.sleep(wait_time)
                        continue
    ...
    async def get_options_chain(self, ticker: str) -> Optional[OptionsChain]:
        ...
        try:
            ...
            yf_ticker, current_price, expiry_dates = await self._make_request_with_retry(get_ticker_info)
            ...
            for expiry_str in expiry_dates[:4]:  # Limit to avoid too many requests
                try:
                    ...
                    logger.warning(f"Rate limit hit while processing {expiry_str} for {ticker}, skipping")
                    continue
                except Exception as e:
                    logger.warning(f"Error processing expiry {expiry_str} for {ticker}: {e}")
                    continue
...
async def get_provider_with_fallback(config: Dict[str, Any]) -> DataProvider:
    ...
    if not isinstance(primary_provider, YFinanceProvider):
        logger.warning(f"Primary provider {primary_provider.name} failed, falling back to YFinance")
