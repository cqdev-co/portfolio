...
async def store_signals(
    config: dict, signals: list, use_continuity: bool = True
) -> None:
    """
    ...
    try:
        if use_continuity:
            ...
            console.print(
                f"[cyan]Processing {len(signals)} signals with continuity tracking...[/cyan]"
            )
            ...
            if stats["failed_signals"] == 0:
                console.print(
                    f"[green]✓ Stored {stats['new_signals']} new, updated {stats['updated_signals']}, marked {expired_count} expired[/green]"
                )
            else:
                console.print(
                    f"[yellow]⚠ Stored {stats['new_signals']} new, updated {stats['updated_signals']}, {stats['failed_signals']} failed[/yellow]"
                )
        else:
            ...
            if success:
                console.print(
                    f"[green]✓ Stored {len(signals)} signals in database[/green]"
                )
            else:
...
async def list_signals(config: dict, days: int, min_grade: str) -> None:
    ...
    try:
        ...
        signals = await storage.get_signals(
            min_grade=min_grade, start_date=start_date, end_date=end_date, limit=100
        )
        ...
        if signals:
            ...
            console.print(
                f"[yellow]No signals found in last {days} days with grade {min_grade}+[/yellow]"
            )
