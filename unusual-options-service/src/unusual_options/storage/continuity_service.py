...
class SignalContinuityService:
    ...
    async def find_existing_signal(
        self, 
            ...
            self.client.rpc(
                'update_signal_continuity',
                {
                    ...
                    'p_new_premium_flow': float(signal.premium_flow) 
                        if signal.premium_flow else 0.0,
    ...
    async def store_new_signal(
        ...
        try:
            ...
            result = self.client.table('unusual_options_signals')\
                .insert(data)\
                .execute()
        ...
        for signal in signals:
            try:
                ...
                if existing_id:
                    ...
                    success = await self.update_existing_signal(
                        existing_id, 
                        signal
                    )
                    if success:
    ...
    async def get_signal_history(
        ...
        """
        try:
            result = self.client.table('unusual_options_signal_continuity')\
                .select('*')\
    ...
    async def get_active_signals(
        ...
        """
        try:
            query = self.client.table('unusual_options_signals')\
                .select('*')\
                .eq('is_active', True)
            ...
            result = query.order('last_detected_at', desc=True)\
                .limit(limit)\
                .execute()
...
async def create_continuity_service(config: Dict[str, Any]) -> SignalContinuityService:
    ...
    if not url or not key:
        raise ValueError(
            "SUPABASE_URL and SUPABASE_KEY must be set in config"
        )
