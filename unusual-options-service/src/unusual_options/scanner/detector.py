...
class AnomalyDetector:
    ...
    def _detect_volume_anomaly(
        ...
        if not historical:
            ...
            if current_volume >= self.min_heuristic_volume:
                ...
                confidence = min(
                    current_volume / 10000, 0.8
                )  # Cap at 0.8 for heuristics
                return Detection(
    ...
    def _detect_premium_flow(
        ...
        if mid_price > 0:
            aggressiveness = (
                (contract.last_price - contract.bid) / spread if spread > 0 else 0.5
            )
        else:
            aggressiveness = 0.5
    ...
    def _detect_unusual_spread(self, contract: OptionsContract) -> Detection | None:
        ...
        if (
            spread_pct < 0.015 and contract.volume > 1000
        ):  # Tighter spread, higher volume
