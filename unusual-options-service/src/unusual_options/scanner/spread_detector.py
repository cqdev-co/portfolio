...
class SpreadDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.min_confidence_to_flag = self.config.get("MIN_SPREAD_CONFIDENCE", 0.80)
        self.min_volume_correlation = self.config.get("MIN_VOLUME_CORRELATION", 0.80)
        self.max_strike_width = self.config.get("MAX_STRIKE_WIDTH", 50)
        self.min_indicators_required = self.config.get("MIN_INDICATORS", 3)
    ...
    def analyze_all_signals(
        ...
        for detection in detections:
            ...
            potential_matches = self._find_matching_contracts(contract, detections)
            ...
            analysis = self._calculate_spread_confidence(contract, potential_matches)

            analyses[symbol] = analysis

        # Log summary
        high_conf = sum(1 for a in analyses.values() if a.spread_confidence >= 0.80)
        med_conf = sum(
            1 for a in analyses.values() if 0.60 <= a.spread_confidence < 0.80
        )
    ...
    def _find_matching_contracts(
        ...
        for detection in all_detections:
            ...
            if (
                other.expiry == contract.expiry
                and other.option_type == contract.option_type
            ):
    ...
    def _calculate_spread_confidence(
        ...
        for match in matches:
            ...
            if match.volume > 0:
                ...
                if self._in_range(volume_ratio, self.tight_volume_ratio):
                    score += 0.40
                    indicators.append(f"Tight volume match: {volume_ratio:.2f}")
                elif self._in_range(volume_ratio, self.moderate_volume_ratio):
                    score += 0.25
                    indicators.append(f"Moderate volume match: {volume_ratio:.2f}")
            ...
            if strike_diff > 0:  # Vertical spread
                if strike_diff <= 10:
                    ...
                    score += 0.08
                    indicators.append(f"Reasonable strikes: ${strike_diff}")

            # 4. Time proximity (10% weight)
            # Both detected in same scan = higher confidence
            time_diff = abs((contract.timestamp - match.timestamp).total_seconds())
            ...
            if contract.implied_volatility and match.implied_volatility:
                iv_diff = abs(contract.implied_volatility - match.implied_volatility)
                if iv_diff < 0.05:
                    score += 0.05
        ...
        if best_match:
            ...
            if contract.last_price > 0 and best_match.last_price > 0:
                net_premium = (
                    abs(contract.last_price - best_match.last_price)
                    * contract.volume
                    * 100
                )
        ...
        return SpreadAnalysis(
            ...
            spread_type=best_type
            if is_likely
            else ("POSSIBLE_SPREAD" if best_confidence >= 0.60 else None),
            matched_contracts=matched_symbols,
            strike_width=strike_width,
            net_premium=net_premium,
            reasoning=best_reasoning
            if best_reasoning
            else ("No strong spread pattern detected"),
