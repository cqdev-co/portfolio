...
async def backfill_spread_detection(
    ...
    if dry_run:
        ...
        high_conf_samples = [
            u for u in updates_to_make 
            if u.get('is_likely_spread')
        ][:10]
        ...
        for i in range(0, len(updates_to_make), batch_size):
            ...
            for update in batch:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to update signal {signal_id}: {e}"
                    )
            
            logger.debug(
                f"Updated {updated_count}/{len(updates_to_make)} signals"
            )
...
def main():
    ...
    parser.add_argument(
        '--limit',
        type=int,
        help='Maximum number of signals to process (for testing)'
    )
    parser.add_argument(
        '--days-back',
        type=int,
    ...
    # Run backfill
    asyncio.run(backfill_spread_detection(
