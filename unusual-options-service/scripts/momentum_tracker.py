...
class MomentumTracker:
    ...
    async def fetch_multi_day_signals(self, days: int = 7, min_grade: str = 'C') -> None:
        ...
        console.print(f"[green]✓ Loaded {len(all_signals)} signals across {len(self.signals_by_day)} days[/green]")
    ...
    def calculate_daily_scores(self) -> Dict[str, Dict[date, float]]:
        ...
        for day, signals in self.signals_by_day.items():
            ...
            for ticker, ticker_sigs in ticker_signals.items():
                ...
                total_premium = sum([s.premium_flow for s in ticker_sigs if s.premium_flow]) / 1000000  # In millions
    ...
    def identify_momentum_patterns(self, ticker_daily_scores: Dict[str, Dict[date, float]]) -> List[MomentumPattern]:
        ...
        for ticker, daily_scores in ticker_daily_scores.items():
            ...
            if pattern_type:
                ...
                for day in ticker_days[-3:]:
                    recent_signals.extend([s for s in self.signals_by_day[day] if s.ticker == ticker])
                
                total_premium = sum([s.premium_flow for s in recent_signals if s.premium_flow])
                
                patterns.append(MomentumPattern(
                    ticker=ticker,
        ...
        return sorted(patterns, key=lambda x: abs(x.momentum_strength) * x.confidence, reverse=True)
    
    def _classify_momentum(self, ticker: str, ticker_days: List[date], 
                          daily_scores: Dict[date, float], momentum_change: float) -> Tuple[str, str, float]:
    ...
    def identify_reversals(self) -> List[Dict[str, Any]]:
        ...
        ticker_periods = defaultdict(lambda: {'previous': {'calls': [], 'puts': []}, 
                                             'recent': {'calls': [], 'puts': []}})
        ...
        for ticker, periods in ticker_periods.items():
            ...
            if abs(prev_ratio - recent_ratio) > 0.4:
                ...
                # Calculate premium
                recent_premium = sum([
                    s.premium_flow for s in 
                    periods['recent']['calls'] + periods['recent']['puts'] 
                    if s.premium_flow
                ])
    ...
    async def run_analysis(self, days: int = 7, min_grade: str = 'C'):
        ...
        if len(self.signals_by_day) < 3:
            console.print("[red]Need at least 3 days of data for momentum analysis.[/red]")
            return
        ...
        # Top opportunities
        accelerating = [p for p in momentum_patterns if p.pattern_type in ['ACCELERATING', 'INCREASING']]
        exhausting = [p for p in momentum_patterns if p.pattern_type in ['EXHAUSTING', 'DECREASING']]
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Track momentum and reversal patterns in options flow")
