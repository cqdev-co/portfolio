...
def main():
    ...
    try:
        result = supabase.table('unusual_options_signals')\
        ...
        for signal in signals:
            ...
            if age.total_seconds() < 0:
                print(f"❌ {ticker}: Timestamp is in the FUTURE! "
                      f"({timestamp_str})")
                issues_found = True
            elif age > timedelta(days=365):
                print(f"⚠️  {ticker}: Timestamp is over 1 year old "
                      f"({age.days} days)")
            elif age > timedelta(days=30):
                print(f"📅 {ticker}: {age.days} days old - "
            ...
            if hour_est < 4 or hour_est > 20:
                print(f"   ⚠️  Detected at {timestamp_est.strftime('%I:%M %p')} EST "
