...
async def cleanup_table(supabase, table_name: str, dry_run: bool = False) -> int:
    ...
    try:
        ...
        if dry_run:
            console.print(
                f"[yellow]Would delete {count} records from {table_name}[/yellow]"
            )
            return count
...
Tables:
  ...
  all         - All tables (default)
    ...
    try:
        ...
        console.print(
            "[yellow]Make sure you have a .env file or environment variables set[/yellow]"
        )
        sys.exit(1)
    ...
    if args.dry_run:
        console.print(
            f"[yellow]DRY RUN: Would delete {total_deleted} total records[/yellow]"
        )
    else:
        console.print(
            f"[green]✓ Cleanup complete! Deleted {total_deleted} total records[/green]"
        )
        console.print(
