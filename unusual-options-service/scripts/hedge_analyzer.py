...
class HedgeAnalyzer:
    ...
    async def fetch_signals(self, days: int = 7, min_grade: str = "A") -> None:
        ...
        console.print(
            f"[blue]Fetching signals (last {days} days, grade {min_grade}+)...[/blue]"
        )
    ...
    def _is_protective_put(
        ...
        if signal.aggressive_order_pct and signal.aggressive_order_pct < 0.50:
    ...
    def _is_index_hedge(
        ...
        if signal.premium_flow and signal.premium_flow >= 2_000_000:
            indicators.append(f"LARGE_PREMIUM_${signal.premium_flow / 1_000_000:.1f}M")
    ...
    def _is_sector_hedge(
        ...
        if signal.premium_flow and signal.premium_flow >= 500_000:
            indicators.append(f"PREMIUM_${signal.premium_flow / 1_000_000:.1f}M")
        ...
        reasoning = (
            (f"Sector hedge: {signal.ticker} put, {signal.days_to_expiry}d DTE.")
            if is_hedge
            else ""
        )
    ...
    def _is_far_dated_mega_cap(
        ...
        if signal.premium_flow and signal.premium_flow >= 2_000_000:
            indicators.append(f"INSTITUTIONAL_${signal.premium_flow / 1_000_000:.1f}M")
    ...
    def _detect_paired_activity(self) -> Dict[str, List[UnusualOptionsSignal]]:
        ...
        for signal in self.signals:
            key = (signal.ticker, signal.detection_timestamp.date())
    ...
    def _collar_confidence(
        self, call: UnusualOptionsSignal, put: UnusualOptionsSignal
    ) -> float:
        ...
        time_diff = abs(
            (call.detection_timestamp - put.detection_timestamp).total_seconds()
        )
        if time_diff < 1800:
            indicators += 1
    ...
    def analyze_all_signals(self) -> None:
        ...
        if paired:
            console.print(
                f"[dim]Found {len(paired)} tickers with paired call/put activity[/dim]"
            )
                ...
                for signal in signals:
                    if signal.signal_id in self.analyses:
                        ...
                        if not analysis.is_likely_hedge:
                            analysis.hedge_indicators.append("PAIRED_CALL_PUT_ACTIVITY")
        ...
        if collars:
            console.print(f"[dim]Found {len(collars)} potential collar patterns[/dim]")
            ...
            for call, put, conf in collars:
                for signal in [call, put]:
                    if signal.signal_id in self.analyses:
                        ...
                        analysis.hedge_confidence = max(analysis.hedge_confidence, conf)
    ...
    def _analyze_single_signal(self, signal: UnusualOptionsSignal) -> HedgeAnalysis:
    ...
    def display_summary(self) -> None:
        ...
        hedge_premium = sum(a.signal.premium_flow or 0 for a in hedges)
        directional_premium = sum(a.signal.premium_flow or 0 for a in directional)
...
"""
        ...
        console.print(
            Panel(
                summary, title="🛡️ Hedge vs. Directional Analysis", border_style="cyan"
            )
        )
        ...
        if hedge_types:
            type_table = Table(title="Hedge Type Breakdown", box=box.ROUNDED)
            type_table.add_column("Type", style="cyan")
            type_table.add_column("Count", justify="right")
            type_table.add_column("Percentage", justify="right")

            for htype, count in sorted(
                hedge_types.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def display_hedge_signals(self, max_rows: int = 20) -> None:
        ...
        hedges = sorted(
            self.get_hedges(), key=lambda x: x.hedge_confidence, reverse=True
        )
        ...
        for analysis in hedges[:max_rows]:
            ...
            indicators_str = "\n".join(analysis.hedge_indicators[:3])

            table.add_row(
                f"{signal.ticker}\n{signal.option_symbol}",
                analysis.hedge_type or "UNKNOWN",
                f"[{conf_style}]{analysis.hedge_confidence:.0%}[/{conf_style}]",
    ...
    def display_directional_signals(self, max_rows: int = 20) -> None:
        ...
        if not directional:
            console.print("[yellow]No directional signals found[/yellow]")
            return
...
async def main():
    ...
    parser.add_argument(
        "--show-hedges-only", action="store_true", help="Only show hedge activity"
    )
    parser.add_argument(
        "--exclude-hedges",
        action="store_true",
