...
class FlowDivergenceAnalyzer:
    ...
    def analyze_size_divergence(self) -> List[Dict[str, Any]]:
        ...
        all_premium_flows = [
            ...
            if s.premium_flow and s.premium_flow > 0
        ]
        ...
        for ticker, flows in ticker_flows.items():
            ...
            large_bias = (
                (large_bullish - large_bearish) / (large_bullish + large_bearish)
                if (large_bullish + large_bearish) > 0
                else 0
            )
            small_bias = (
                (small_bullish - small_bearish) / (small_bullish + small_bearish)
                if (small_bullish + small_bearish) > 0
                else 0
            )

            # Divergence if large and small money are going opposite directions
            if (large_bias > 0.3 and small_bias < -0.3) or (
                large_bias < -0.3 and small_bias > 0.3
            ):
                divergences.append(
                    {
                        "ticker": ticker,
                        ...
                        "large_premium": sum(
                            [s.premium_flow for s in large_signals if s.premium_flow]
                        ),
                        "small_premium": sum(
                            [s.premium_flow for s in small_signals if s.premium_flow]
                        ),
                        "interpretation": self._interpret_size_divergence(
                            large_bias, small_bias
                        ),
    ...
    def analyze_time_divergence(self) -> List[Dict[str, Any]]:
        ...
        for ticker, times in ticker_times.items():
            ...
            near_call_premium = sum(
                [
                    s.premium_flow
                    for s in near_signals
                    if s.option_type == "call" and s.premium_flow
                ]
            )
            near_put_premium = sum(
                [
                    s.premium_flow
                    for s in near_signals
                    if s.option_type == "put" and s.premium_flow
                ]
            )
            far_call_premium = sum(
                [
                    s.premium_flow
                    for s in far_signals
                    if s.option_type == "call" and s.premium_flow
                ]
            )
            far_put_premium = sum(
            ...
            if (near_call_ratio > 0.6 and far_call_ratio < 0.4) or (
                near_call_ratio < 0.4 and far_call_ratio > 0.6
            ):
                divergences.append(
                    {
                        "ticker": ticker,
                        ...
                        "interpretation": self._interpret_time_divergence(
                            near_call_ratio, far_call_ratio
                        ),
                ...
                divergences.append(
                    {
                        ...
                        "interpretation": self._interpret_moneyness_divergence(
                            itm_ratio
                        ),
    ...
    def display_moneyness_divergence(self, divergences: List[Dict[str, Any]]) -> None:
        ...
        if not divergences:
            console.print(
                "[yellow]No significant moneyness divergences found.[/yellow]"
            )
            return
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze options flow divergence patterns"
    )
    parser.add_argument(
        "--days", type=int, default=3, help="Number of days to analyze (default: 3)"
    )
    parser.add_argument(
