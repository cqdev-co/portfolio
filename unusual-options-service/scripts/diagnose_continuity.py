...
class ContinuityDiagnostics:
    ...
    def check_duplicates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if duplicates:
                console.print(
                    f"[red]✗ Found {len(duplicates)} duplicate "
                    f"signal groups[/red]"
                )
    ...
    def check_detection_counts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result.data:
                console.print(
                    "[yellow]⚠ No signals with detection_count > 1 "
                    "found[/yellow]"
                )
                console.print(
            ...
            console.print(
                f"[green]✓ Found {len(signals)} re-detected "
                f"signals[/green]"
            )
            ...
            for sig in signals[:10]:
                first = sig['first_detected_at'][:16] if sig.get(
                    'first_detected_at'
                ) else 'N/A'
                last = sig['last_detected_at'][:16] if sig.get(
                    'last_detected_at'
                ) else 'N/A'
            ...
            console.print(
                f"[red]✗ Error checking detection counts: {e}[/red]"
            )
    ...
    def check_timestamp_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for sig in signals:
                ...
                # Check first <= last
                if sig.get('first_detected_at') and sig.get(
                    'last_detected_at'
                ):
                ...
                if sig.get('detection_timestamp') and sig.get(
                    'last_detected_at'
                ):
            ...
            if issues:
                console.print(
                    f"[yellow]⚠ Found {len(issues)} timestamp "
                    f"issues[/yellow]"
                )
                ...
                console.print(
                    "[green]✓ All timestamps consistent![/green]"
                )
            ...
            console.print(
                f"[red]✗ Error checking timestamps: {e}[/red]"
            )
    ...
    def check_active_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Count inactive
            inactive_result = client.table('unusual_options_signals')\
                .select('*', count='exact')\
                .eq('is_active', False)\
                .execute()
            ...
            recently_inactive = client.table('unusual_options_signals')\
                .select('*')\
                .eq('is_active', False)\
                .gte(
                    'last_detected_at',
            ...
            recent_count = len(
                recently_inactive.data
            ) if recently_inactive.data else 0
            ...
            console.print(
                f"[red]✗ Error checking active status: {e}[/red]"
            )
            return {
                'active_count': 0,
                'inactive_count': 0,
    ...
    def check_continuity_history(
        ...
        try:
            ...
            if continuity_count == 0:
                console.print(
                    "[yellow]⚠ No continuity history found[/yellow]"
                )
                console.print(
                    "   This is normal if signals haven't been "
                    "re-detected yet"
                )
                return {'continuity_records': 0}
            
            console.print(
                f"[green]✓ Found {continuity_count} continuity "
                f"records[/green]"
            )
            
            # Get sample with history
            query = client.table('unusual_options_signals')\
                .select('*')\
                .gt('detection_count', 1)
            ...
            return {
                ...
                'signals_with_history': len(
                    signals_result.data
                ) if signals_result.data else 0
            }
            
        except Exception as e:
            console.print(
                f"[red]✗ Error checking continuity history: {e}[/red]"
            )
    ...
    def run_full_diagnostics(
        ...
        console.print(
            Panel(
                "[bold cyan]Unusual Options Signal Continuity "
        ...
        results['continuity_history'] = self.check_continuity_history(
            ticker
        )
        ...
        if results['duplicates']['count'] > 0:
            critical_issues.append(
                f"❌ {results['duplicates']['count']} duplicate "
                f"signal groups"
            )
        
        if results['active_status']['active_count'] == 0 and \
           results['active_status']['total'] > 0:
            critical_issues.append(
                "❌ All signals inactive (likely timestamp issue)"
            )
        ...
        if results['detection_counts']['count'] == 0:
            console.print(
                "  • Run the scanner twice to test deduplication"
            )
        
        if results['continuity_history']['continuity_records'] == 0:
            console.print(
                "  • Re-run scanner to build continuity history"
            )
        
        if results['duplicates']['count'] > 0:
            console.print(
                "  • Run cleanup script and re-scan with fixed "
                "continuity service"
            )
...
@click.option(
    '--ticker',
    '-t',
    help='Filter diagnostics to specific ticker'
)
@click.option(
    '--verbose',
    '-v',
...
def main(ticker: str = None, verbose: bool = False):
    ...
    try:
        ...
        if results['duplicates']['count'] > 0 or \
           (results['active_status']['active_count'] == 0 and 
            results['active_status']['total'] > 0):
            sys.exit(1)
