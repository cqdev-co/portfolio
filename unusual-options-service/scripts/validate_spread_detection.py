...
def get_price_change(ticker: str, start_date: datetime, days_forward: int = 5) -> Optional[float]:
...
async def validate_spread_detection(days_back: int = 14):
    ...
    high_conf_spreads = [
        s for s in signals 
        if s.get('is_likely_spread') == True
    ]
    medium_conf_spreads = [
        s for s in signals 
    ...
    directional_signals = [
        s for s in signals 
        if s.get('spread_confidence', 0) < 0.60
    ]
    ...
    high_conf_stats = analyze_category(
        high_conf_spreads, 
        "High-Confidence Spreads (≥80%)"
    )
    medium_conf_stats = analyze_category(
        medium_conf_spreads,
        "Medium-Confidence Spreads (60-79%)"
    )
    ...
    if high_conf_stats and directional_stats:
        ...
        if false_positive_rate < 20:
            ...
            logger.info("   Recommendation: Keep tagging only, monitor for 1-2 more weeks")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate spread detection accuracy'
    )
    parser.add_argument(
        '--days-back',
        type=int,
