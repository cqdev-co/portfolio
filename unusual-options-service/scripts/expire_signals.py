...
def expire_signals(dry_run: bool = False) -> dict:
    ...
    try:
        ...
        logger.info(f"\nTop 10 Tickers:")
        top_tickers = sorted(by_ticker.items(), 
                            key=lambda x: x[1], 
                            reverse=True)[:10]
        ...
        if not dry_run:
            ...
            for i in range(0, len(signal_ids), batch_size):
                ...
                update_result = client.table('unusual_options_signals') \
                ...
                if update_result.data:
                    ...
                    logger.debug(
                        ...
                        f"{batch_count} signals"
                    )
            
            logger.success(
                f"✓ Successfully expired {total_updated:,} signals"
            )
            ...
            logger.info(
                "\n[DRY RUN] Would have expired "
                f"{len(expired_signals):,} signals"
            )
            return {
                'success': True,
                'signals_found': len(expired_signals),
