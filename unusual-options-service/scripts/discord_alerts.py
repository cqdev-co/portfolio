...
async def send_insider_play_alerts(days: int = 1, min_score: float = 60):
    ...
    if not notifier.is_configured:
        console.print(
            "[red]❌ DISCORD_WEBHOOK_URL not configured[/red]"
        )
        return
    
    console.print(
        f"[blue]Fetching signals from last {days} day(s)...[/blue]"
    )
    ...
    for signal, score in high_conviction[:5]:
        ...
        if success:
            sent += 1
            console.print(
                f"[green]✓ Sent alert for {signal.ticker}[/green]"
            )
...
async def send_performance_report_alert():
    ...
    hedge_signals = [
        s for s in signals
        if s.ticker in INDEX_ETFS or s.ticker in SECTOR_ETFS
        or (s.ticker in MEGA_CAPS and s.option_type == 'put' 
            and s.days_to_expiry and s.days_to_expiry >= 45)
    ]
...
async def main():
    ...
    parser.add_argument(
        "--insider-plays", action="store_true",
    ...
    parser.add_argument(
        ...
        help="Send daily market close summary"
    )
    parser.add_argument(
