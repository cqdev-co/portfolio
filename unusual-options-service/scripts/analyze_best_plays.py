...
class SignalAnalyzer:
    ...
    def apply_consistency_filter(
        ...
        filtered = [
            s for s in signals 
            if s['ticker'] in high_consistency_tickers
        ]
...
def print_top_plays(results: Dict, top_n: int = 10):
    ...
    for i, (signal, rank_score) in enumerate(signals, 1):
        ...
        print(f"   Rank: {rank_score:.1f} | Score: {score:.2f} | "
              f"Grade: {grade} | Conf: {confidence:.2f}")
        print(f"   Premium: ${premium:,.0f} | Underlying: ${underlying:.2f} | "
              f"{moneyness}")
        ...
        if underlying > 0:
            ...
            if option_type == 'CALL':
                if distance < 0:
                    print(f"   💚 Already ITM by ${abs(strike-underlying):.2f} "
                          f"({abs(distance):.1f}%)")
                else:
...
def print_ticker_summary(results: Dict):
    ...
    for ticker, cons_score in sorted_tickers[:15]:
        ...
        total_premium = sum(
            float(s.get('premium_flow', 0) or 0) 
            for s, _ in signals_list
        )
        
        direction = "📈 BULLISH" if calls > puts else "📉 BEARISH" if puts > calls else "⚖️  MIXED"
        
        print(f"\n{ticker} - Consistency: {cons_score}/10")
        print(f"  {direction} | Signals: {len(signals_list)} "
...
def print_strategy_recommendations(results: Dict):
    ...
    tech_tickers = ['AAPL', 'GOOGL', 'GOOG', 'MSFT', 'AMD', 'NVDA', 
                    'TSM', 'AVGO', 'QCOM', 'INTC']
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Minimum days to expiry (default: 10)'
    )
    parser.add_argument(
        '--min-premium',
        type=float,
    ...
    try:
        ...
        print("3. Size positions according to tier (Tier 1: 3-5%, "
              "Tier 2: 2-3%, Tier 3: 0.5-1%)")
        print("4. Set stop losses at -20% to -25%")
        print("5. Take profits at 50% (half position), 100% (quarter), "
              "trail remainder")
        print("\n⚠️  Remember: These are signals, not guarantees. "
              "Trade responsibly!")
