...
class TradeSizingCalculator:
    ...
    async def fetch_signals(self, days: int = 7, min_grade: str = "B") -> None:
        ...
        for signal in self.signals:
            if (
                ...
                and signal.current_volume >= 100
                and signal.days_to_expiry
                and 7 <= signal.days_to_expiry <= 60
                and signal.premium_flow
                and signal.premium_flow >= 50000
            ):
    ...
    def estimate_option_pricing(
        ...
        time_factor = (
            max(0.5, signal.days_to_expiry / 30) if signal.days_to_expiry else 0.5
        )
        estimated_price *= time_factor

        # Estimate target based on historical performance
        grade_perf = self.historical_performance.get(
            signal.grade, self.historical_performance["C"]
        )
    ...
    def calculate_position_sizing(self, signal: UnusualOptionsSignal) -> PositionSizing:
        ...
        kelly_fraction = self.calculate_kelly_fraction(
            adj_win_rate, adj_avg_win, adj_avg_loss
        )
        ...
        contracts_by_risk = max_loss_amount / (
            entry_price * 100
    ...
    def run_monte_carlo_simulation(
        ...
        console.print(
            f"[blue]Running {num_simulations} Monte Carlo simulations...[/blue]"
        )
        ...
        with Progress(
            ...
            for i in range(num_simulations):
                ...
                for position in positions:
                    grade_perf = self.historical_performance.get(
                        position.signal.grade, self.historical_performance["C"]
                    )

                    # Simulate trade outcome
                    if (
                        random.random()
                        < grade_perf["win_rate"] * position.confidence_score
                    ):
                        # Win
                        return_pct = random.normalvariate(
                            grade_perf["avg_win"], grade_perf["volatility"] * 0.5
                        )
                    else:
                        # Loss
                        return_pct = random.normalvariate(
                            grade_perf["avg_loss"], grade_perf["volatility"] * 0.3
                        )
                ...
                results.append(
                    portfolio_return / self.params.account_size
        ...
        return {
            ...
            "sharpe_ratio": statistics.mean(results) / statistics.stdev(results)
            if statistics.stdev(results) > 0
            else 0,
        }
    ...
    def display_position_recommendations(self, positions: List[PositionSizing]) -> None:
        ...
        for position in sorted(positions, key=lambda x: x.expected_value, reverse=True)[
            :20
        ]:
            grade_style = {
                "S": "bold green",
            ...
            warnings_text = (
                ", ".join(position.warnings[:2]) if position.warnings else "None"
            )
        ...
        risk_color = (
            "green" if risk_pct < 0.05 else "yellow" if risk_pct < 0.08 else "red"
        )
    ...
    def display_monte_carlo_results(self, mc_results: Dict[str, Any]) -> None:
        ...
        table.add_row(
            "Expected Return",
            f"{mc_results['mean_return']:.2%}",
            "Average portfolio return",
        )
        table.add_row(
            "Median Return",
            f"{mc_results['median_return']:.2%}",
            "50th percentile outcome",
        )
        table.add_row(
            "Standard Deviation", f"{mc_results['std_dev']:.2%}", "Return volatility"
        )
        table.add_row(
            "95% VaR", f"{mc_results['var_95']:.2%}", "5% chance of losing more"
        )
        table.add_row(
            "99% VaR", f"{mc_results['var_99']:.2%}", "1% chance of losing more"
        )
        table.add_row(
            "Maximum Loss", f"{mc_results['max_loss']:.2%}", "Worst case scenario"
        )
        table.add_row(
            "Maximum Gain", f"{mc_results['max_gain']:.2%}", "Best case scenario"
        )
        table.add_row(
            "Probability of Profit",
            f"{mc_results['prob_profit']:.1%}",
            "Chance of positive return",
        )
        table.add_row(
            "Sharpe Ratio", f"{mc_results['sharpe_ratio']:.2f}", "Risk-adjusted return"
        )
        ...
        risk_assessment = (
            "LOW"
            if mc_results["var_95"] > -0.05
            else "MEDIUM"
            if mc_results["var_95"] > -0.10
            else "HIGH"
        )
    ...
    async def run_analysis(self, days: int = 7, min_grade: str = "B"):
        ...
        if not positions:
            console.print(
                "[yellow]No positions with positive expected value found.[/yellow]"
            )
            return
        ...
        top_positions = sorted(positions, key=lambda x: x.expected_value, reverse=True)[
            :10
        ]
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate optimal position sizes for unusual options signals"
    )
    parser.add_argument(
        "--account-size",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Max risk per trade as decimal (default: 0.02)",
    )
    parser.add_argument(
        "--days", type=int, default=7, help="Number of days to analyze (default: 7)"
    )
    parser.add_argument(
        "--min-grade", type=str, default="B", help="Minimum signal grade (default: B)"
    )

    args = parser.parse_args()

    trading_params = TradingParameters(
        account_size=args.account_size, max_position_risk=args.max_risk
    )
