...
class SignalCorrelationAnalyzer:
    def __init__(self):
        ...
        self.sector_map = {
            ...
            'META': 'Technology', 'NVDA': 'Technology', 'AMD': 'Technology', 'CRM': 'Technology',
            'ORCL': 'Technology', 'ADBE': 'Technology', 'INTC': 'Technology', 'CSCO': 'Technology',
            ...
            'NKE': 'Consumer', 'SBUX': 'Consumer', 'TGT': 'Consumer', 'WMT': 'Consumer',
        ...
        self.market_cap_tiers = {
            ...
            'META': 'Mega', 'TSLA': 'Large', 'JPM': 'Large', 'JNJ': 'Large', 'PG': 'Large',
        }
    ...
    def calculate_ticker_correlations(self) -> List[CorrelationPair]:
        ...
        active_tickers = {ticker: signals for ticker, signals in ticker_signals.items() 
                         if len(signals) >= 3}
        ...
        for i, ticker1 in enumerate(tickers):
            for ticker2 in tickers[i+1:]:
                correlation = self._calculate_pair_correlation(
                    active_tickers[ticker1], 
                    active_tickers[ticker2],
                    ticker1,
                    ticker2
                )
                if correlation:
    ...
    def _calculate_pair_correlation(self, signals1: List[UnusualOptionsSignal], 
                                  signals2: List[UnusualOptionsSignal],
                                  ticker1: str, ticker2: str) -> Optional[CorrelationPair]:
        ...
        for signal in signals1:
            bucket = signal.detection_timestamp.replace(minute=0, second=0, microsecond=0)
        ...
        if len(values1) < 3 or all(v == 0 for v in values1) or all(v == 0 for v in values2):
            return None
    ...
    def _count_shared_characteristics(self, signals1: List[UnusualOptionsSignal], 
                                    signals2: List[UnusualOptionsSignal]) -> int:
        ...
        for s1 in signals1:
            for s2 in signals2:
                ...
                time_diff = abs((s1.detection_timestamp - s2.detection_timestamp).total_seconds())
                if time_diff < 3600:  # Within 1 hour
                    # Check for shared characteristics
                    if (s1.option_type == s2.option_type and
                        s1.sentiment == s2.sentiment and
                        abs(s1.overall_score - s2.overall_score) < 0.2):
                        shared += 1
                        break
    ...
    def _cluster_by_sector(self) -> List[SignalCluster]:
        ...
        for sector, signals in sector_signals.items():
            if len(signals) >= 5:  # Minimum cluster size
                ...
                avg_premium = statistics.mean([s.premium_flow for s in signals if s.premium_flow])
                ...
                if avg_score > 0.8 and len(signals) > 10:
                    ...
                    recommendation = f"WEAK_SECTOR_SIGNAL: {sector} activity may be noise"
    ...
    def _cluster_by_premium_flow(self) -> List[SignalCluster]:
        ...
        premium_signals = [s for s in self.signals if s.premium_flow and s.premium_flow > 0]
        ...
        large_flow_signals = [s for s in premium_signals if s.premium_flow >= large_threshold]
        medium_flow_signals = [s for s in premium_signals 
        ...
        if len(large_flow_signals) >= 3:
            clusters.append(SignalCluster(
                ...
                center_characteristics={
                    ...
                    'avg_premium': statistics.mean([s.premium_flow for s in large_flow_signals]),
        ...
        if len(medium_flow_signals) >= 5:
            clusters.append(SignalCluster(
                ...
                center_characteristics={
                    ...
                    'avg_premium': statistics.mean([s.premium_flow for s in medium_flow_signals]),
        ...
        for key, signals in expiry_moneyness_signals.items():
            if len(signals) >= 5:
                ...
                if expiry_bucket == "SHORT_TERM" and moneyness == "OTM":
                    risk_level = "HIGH"
                    recommendation = "HIGH_RISK: Short-term OTM options are highly speculative"
                elif expiry_bucket == "LONG_TERM" and moneyness == "ITM":
                    risk_level = "LOW"
                    recommendation = "CONSERVATIVE: Long-term ITM options offer lower risk"
                else:
                    risk_level = "MEDIUM"
                ...
                clusters.append(SignalCluster(
                    ...
                    center_characteristics={
                        ...
                        'moneyness': moneyness,
                        'avg_days_to_expiry': statistics.mean([s.days_to_expiry for s in signals if s.days_to_expiry]),
                        'signal_count': len(signals)
                    },
    ...
    def analyze_market_regime(self) -> Dict[str, Any]:
        ...
        sentiment_ratio = bullish_signals / (bullish_signals + bearish_signals) if (bullish_signals + bearish_signals) > 0 else 0.5
    ...
    def display_clusters(self, clusters: List[SignalCluster]) -> None:
        ...
        for cluster_type, type_clusters in cluster_types.items():
            ...
            for cluster in sorted(type_clusters, key=lambda x: len(x.signals), reverse=True):
                risk_color = {"LOW": "green", "MEDIUM": "yellow", "HIGH": "red"}[cluster.risk_level]
                
                # Format key metrics based on cluster type
                if cluster_type == "SECTOR":
                    metrics = f"Avg Score: {cluster.center_characteristics['avg_score']:.2f}"
                ...
                table.add_row(
                    ...
                    metrics,
                    cluster.recommendation.split(':')[1].strip() if ':' in cluster.recommendation else cluster.recommendation
                )
    ...
    def display_market_regime(self, regime_analysis: Dict[str, Any]) -> None:
        ...
        table.add_row("Current Regime", f"[{regime_color}]{regime.replace('_', ' ')}[/{regime_color}]", "Based on options flow patterns")
        table.add_row("Confidence", f"{confidence:.1%}", "Reliability of regime identification")
        table.add_row("Sentiment Ratio", f"{regime_analysis['sentiment_ratio']:.1%}", "Bullish vs Bearish signals")
        table.add_row("High-Grade Ratio", f"{regime_analysis['high_grade_ratio']:.1%}", "Quality signal percentage")
        table.add_row("Avg Premium Flow", f"${regime_analysis['avg_premium_flow']:,.0f}", "Average institutional activity")
        table.add_row("Risk Ratio", f"{regime_analysis['risk_ratio']:.1%}", "High-risk signal percentage")
        ...
        interpretation = interpretations.get(regime, "Regime unclear - monitor for pattern development.")
        ...
        console.print(Panel.fit(
            ...
            border_style=regime_color.split()[1] if ' ' in regime_color else regime_color
        ))
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze signal correlations and clustering patterns")
    parser.add_argument("--days", type=int, default=14, help="Number of days to analyze (default: 14)")
    parser.add_argument("--min-grade", type=str, default="C", help="Minimum signal grade (default: C)")
