...
class InsiderPlayDetector:
    ...
    async def fetch_signals(
        self, days: int = 3, min_grade: str = "A", strict_mode: bool = True
    ) -> None:
        """
        ...
        if strict_mode:
            ...
            self.signals = [
                s for s in self.signals if s.days_to_expiry and s.days_to_expiry >= 3
            ]
            filtered = original_count - len(self.signals)
            if filtered > 0:
                console.print(f"[dim]Filtered {filtered} 0-2 DTE signals[/dim]")
    ...
    def _apply_quality_filters(self) -> None:
        ...
        if total_filtered > 0:
            console.print(
                f"[dim]  • ETFs (hedge targets): {filter_stats['etf_filtered']}[/dim]"
            )
            console.print(
                f"[dim]  • DTE outside 7-45d: {filter_stats['dte_filtered']}[/dim]"
            )
            console.print(
                f"[dim]  • Premium < $2M: {filter_stats['premium_filtered']}[/dim]"
            )
            console.print(
                f"[dim]  • Mega-cap < $10M: {filter_stats['mega_cap_filtered']}[/dim]"
            )
            console.print(
    ...
    def _get_or_create_play(self, signal: UnusualOptionsSignal) -> InsiderPlay:
        ...
        if signal.signal_id not in self.plays_by_signal_id:
            play = InsiderPlay(
                signal=signal, surprise_factor=getattr(signal, "surprise_factor", 0.0)
            )
    ...
    def detect_large_concentrated_bets(self) -> List[InsiderPlay]:
        ...
        for signal in self.signals:
            ...
            if signal.moneyness in ["ITM", "ATM"]:
                ...
                suspicion = min(
                    signal.premium_flow / 10_000_000 * 100, 100
                )  # Scale to 100
    ...
    def detect_volume_to_oi_anomalies(self) -> List[InsiderPlay]:
        ...
        for signal in self.signals:
            ...
            if (
                ...
                and signal.premium_flow >= 1_000_000
            ):
    ...
    def detect_earnings_plays(self) -> List[InsiderPlay]:
        ...
        for signal in self.signals:
            ...
            if (
                days_until_earnings_date is not None
                and 0 <= days_until_earnings_date <= 14
            ):
    ...
    def get_filtered_plays(self) -> List[InsiderPlay]:
        ...
        if self.exclude_hedges:
            ...
            plays = [p for p in plays if not self._is_likely_hedge(p.signal)]
            filtered_count = original_count - len(plays)
            if filtered_count > 0:
                console.print(
                    f"[dim]Filtered out {filtered_count} likely hedge positions[/dim]"
                )
        ...
        # the suspicion score
        for play in plays:
            play.composite_score = play.suspicion_score * (
                1 + play.surprise_factor / 10
            )
    ...
    def display_insider_plays(
        ...
        for play in plays[:max_rows]:
            ...
            patterns_str = "\n".join(
                [pattern_names.get(p, p) for p in sorted(play.play_types)]
            )
            ...
            if "premium" in play.key_metrics:
                metrics_items.append(
                    f"Premium: ${play.key_metrics['premium'] / 1_000_000:.1f}M"
                )
    ...
    async def run_analysis(
        ...
        if len(self.signals) < 5:
            console.print(
                "[yellow]Not enough high-quality signals for analysis.[/yellow]"
            )
            return
        ...
        if high_suspicion:
            self.display_insider_plays(
                high_suspicion, "🔥 HIGH SUSPICION PLAYS (Score ≥80)", max_rows=15
            )
            console.print()
...
async def main():
    ...
    parser.add_argument(
        "--days", type=int, default=3, help="Number of days to scan (default: 3)"
    )
    parser.add_argument(
