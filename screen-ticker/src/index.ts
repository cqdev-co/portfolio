...
program
  ...
  .option("-p, --position <spread>", "Your position (e.g., '111/112 Call Debit Spread')")
  .option("--ai-mode <mode>", "Ollama mode: local or cloud (default: cloud)", "cloud")
  ...
  .action(async (ticker: string, opts: { 
    ...
    console.log(chalk.gray(`  AI mode: ${opts.aiMode}${opts.aiModel ? ` (${opts.aiModel})` : ""}`));
    console.log();

    // Validate AI is available before proceeding
    const aiValidation = await validateAIRequirement(opts.aiMode as OllamaMode);
    ...
    try {
      ...
      const parsedPosition = opts.position ? parsePosition(opts.position) : null;
      if (opts.position && !parsedPosition) {
        logger.warn(`Could not parse position: "${opts.position}"`);
        logger.info(`Expected format: "111/112 Call Debit Spread" or "115/110 Put Credit Spread"`);
      }
      ...
      let aiAnalysis: AIAnalysisResult | null = null;
      {
        ...
        const trend = result.historical.length > 50
          ? analyzeTrend(result.historical, result.score.price)
          : null;
        const rsi = result.historical.length > 20
          ? generateRSILine(result.historical)
          : null;
        const levels = result.historical.length > 30
        ...
        const aiContext = createAIContext(
          ...
          aiLevels,
          positionContext,
          Object.keys(catalystContext).length > 0 ? catalystContext : undefined
        );
 ...
 * Returns ENTER, WAIT, or PASS based on available data
 */
type QuickDecision = "ENTER" | "WAIT" | "PASS";
...
function getQuickDecision(score: StockScore): QuickDecisionResult {
  ...
  // Check 3: Analyst sentiment (look for upgrade/revision signals)
  const analystSignals = score.signals.filter(s => 
    s.category === "analyst" && s.points > 0
  );
  if (analystSignals.length >= 2) {
    checks.analystPositive = true;
  } else {
    // Check warnings for negative analyst sentiment
    const analystWarning = score.warnings?.find(w =>
  ...
  // Critical: Below MA200 forces WAIT at minimum (never ENTER)
  if (belowMA200) {
    decision = passedChecks >= 3 ? "WAIT" : "PASS";
    reason = "Below MA200";
  } else if (passedChecks >= 4 && checks.aboveMA200 && checks.rsiOK) {
    decision = "ENTER";
    reason = `${passedChecks}/${totalChecks} checks`;
  } else if (passedChecks >= 3 && (checks.aboveMA200 || checks.scoreHigh)) {
    decision = "WAIT";
  ...
  return { decision, reason, checksPass: passedChecks, checksTotal: totalChecks };
}
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      console.log(
        ...
        chalk.green(`${enterCount} ENTER`) + chalk.gray(" | ") +
        chalk.yellow(`${waitCount} WAIT`) + chalk.gray(" | ") +
...
function displayNarrativeAnalysis(
  ...
  const { 
    ticker, name, price, technicalScore, fundamentalScore, 
    analystScore, totalScore, upsidePotential, signals 
  } = score;

  // Group signals
  const technicalSignals = signals.filter(s => s.category === "technical");
  const fundamentalSignals = signals.filter(s => s.category === "fundamental");
  const analystSignals = signals.filter(s => s.category === "analyst");

  // Analyze trend and RSI
  const trend = historical.length > 50 
    ? analyzeTrend(historical, price) 
    : null;
  const rsi = historical.length > 20 
    ? generateRSILine(historical) 
    : null;
  const volume = historical.length > 20 
    ? analyzeVolume(historical) 
    : null;
  const levels = historical.length > 30 
    ? getKeyLevels(historical, price) 
    : null;
  ...
  if (userPosition && levels) {
    ...
    // Current status
    const statusEmoji = posAnalysis.riskLevel === "low" 
      ? "🟢" 
      : posAnalysis.riskLevel === "medium" 
        ...
        : posAnalysis.riskLevel === "high"
          ? "🟠"
          : "🔴";
    const statusColor = posAnalysis.riskLevel === "low" 
      ? chalk.green 
      : posAnalysis.riskLevel === "medium" 
        ? chalk.yellow 
        : posAnalysis.riskLevel === "high"
    ...
    const recColor = posAnalysis.riskLevel === "low" 
      ? chalk.green 
      : posAnalysis.riskLevel === "medium"
        ? chalk.yellow
        : chalk.red;
    console.log(
      chalk.gray("  → ") + 
      recColor(posAnalysis.recommendation)
    );
    console.log();
  ...
  if (marketContext) {
    ...
    const regimeEmoji = marketContext.regime === "bull" 
      ? "🟢" 
      : marketContext.regime === "bear" 
        ? "🔴" 
        : "🟡";
    const regimeColor = marketContext.regime === "bull" 
      ? chalk.green 
      : marketContext.regime === "bear" 
        ? chalk.red 
        : chalk.yellow;
    ...
    if (marketContext.regime === "bull") {
      console.log(
        chalk.gray("  → ") + 
        chalk.white(marketContext.recommendation)
      );
    } else if (marketContext.regime === "bear") {
      console.log(
        chalk.gray("  → ") + 
        chalk.yellow(marketContext.recommendation)
      );
    } else {
      console.log(
        chalk.gray("  → ") + 
  ...
  if (score.context && (score.context.low52 || score.context.high52)) {
    ...
    if (ctx.low52 && ctx.high52) {
      ...
      console.log(`  52-Week Range: $${ctx.low52.toFixed(2)} ${rangeBar} $${ctx.high52.toFixed(2)}`);
      ...
      if (ctx.pctFromLow !== undefined && ctx.pctFromHigh !== undefined) {
        ...
        console.log(
          chalk.gray(`               +${fromLow}% from low  |  -${fromHigh}% from high`)
        );
      }
      console.log();
    }
    ...
    if (ctx.nextEarningsDate) {
      ...
      if (days <= 14) {
        console.log(
          `  Next Earnings: ${dateStr} (${days} days) ` +
          chalk.yellow("⚠️")
        );
      } else {
  ...
  if (quarterlyPerformance && quarterlyPerformance.quarters.length > 0) {
    ...
    if (revDisplay.formatted.includes("$")) {
      const revTrendEmoji = qp.revenueTrend === "growing" ? "📈" :
                            qp.revenueTrend === "declining" ? "📉" : "📊";
      const revTrendColor = qp.revenueTrend === "growing" ? chalk.green :
                            qp.revenueTrend === "declining" ? chalk.red : 
                            chalk.yellow;
      console.log(
        chalk.gray("  Revenue:    ") + 
    ...
    if (earnDisplay.formatted.includes("$")) {
      const earnTrendEmoji = qp.earningsTrend === "improving" ? "📈" :
                             qp.earningsTrend === "declining" ? "📉" : "📊";
      const earnTrendColor = qp.earningsTrend === "improving" ? chalk.green :
                             qp.earningsTrend === "declining" ? chalk.red : 
                             chalk.yellow;
      console.log(
        chalk.gray("  Earnings:   ") + 
    ...
    // Beat/miss summary
    if (bmRecord.total > 0) {
      const bmColor = bmRecord.beats >= bmRecord.misses ? chalk.green : 
                      chalk.yellow;
    ...
    if (qp.surpriseTrend === "consistently_beating") {
      console.log(
        `  ${chalk.bold("Insight:")} ` +
        chalk.green("Consistently beating estimates — management under-promises")
      );
  ...
  if (relativeStrength && relativeStrength.rs20) {
    ...
    console.log(
      `  Overall: ` +
      trendColor(`${trend.charAt(0).toUpperCase() + trend.slice(1)} relative strength ${trendEmoji}`)
    );
    console.log();
  }
  ...
  if (momentum && momentum.signals.length > 0) {
    ...
    console.log(`  Overall: ` + summaryColor(`${momentum.summary} ${summaryEmoji}`));
    console.log();
  }
  ...
  if (showChart && historical.length > 30) {
    ...
    // Trend summary
    if (trend) {
      const trendIcon = trend.direction === "bullish" 
        ? "↑" 
        : trend.direction === "bearish" 
          ? "↓" 
          : "→";
      const trendColor = trend.direction === "bullish" 
        ? chalk.green 
        : trend.direction === "bearish" 
          ? chalk.red 
          : chalk.yellow;
    ...
    // Volume
    if (volume) {
      const volIcon = volume.trend === "increasing" 
        ? "📈" 
        : volume.trend === "decreasing" 
          ? "📉" 
          : "📊";
      console.log(`  ${chalk.bold("Volume:")} ${volIcon} ${volume.description}`);
    }
  ...
  const bullSignals = [...technicalSignals, ...fundamentalSignals, ...analystSignals]
    .filter(s => s.points >= 3)
  ...
  if (bullSignals.length === 0) {
    ...
    bullSignals.slice(0, 6).forEach(s => {
      const categoryIcon = s.category === "technical" 
        ? "📊" 
  ...
  if (warnings.length > 0) {
    warnings.forEach(w => {
      console.log(chalk.red(`  🚨 ${w.name}`) + chalk.gray(` — ${w.description}`));
    });
  }
  
  const concerns = generateConcerns(score, trend, rsi, momentum, relativeStrength);
  ...
  if (score.dataQuality === "poor") {
    console.log(chalk.gray("     ") + chalk.red("⚠ Limited fundamental data available"));
  } else if (score.dataQuality === "partial") {
    console.log(chalk.gray("     ") + chalk.yellow("ℹ Some fundamental metrics missing"));
  }
  ...
  if (score.context?.sector) {
    ...
    if (comp) {
      const compColor = comp.type === "bullish" ? chalk.green : 
                        comp.type === "bearish" ? chalk.yellow : chalk.gray;
      const icon = comp.type === "bullish" ? "✓" : "⚠";
      console.log(chalk.gray(`     vs ${ctx.sector}: `) + 
  ...
  if (aiAnalysis?.unified) {
    ...
    const recColor = ai.recommendation === "BUY" 
      ? chalk.green 
      : ai.recommendation === "AVOID" 
        ? chalk.red 
        : chalk.yellow;
    const recEmoji = ai.recommendation === "BUY" 
      ? "🟢" 
      : ai.recommendation === "AVOID" 
        ? "🔴" 
        : "🟡";
    
    const confColor = ai.confidence === "HIGH" 
      ? chalk.green 
      : ai.confidence === "LOW" 
        ? chalk.red 
        : chalk.yellow;
    ...
    if (ai.positionAdvice) {
      ...
      const actionColor = pa.action === "HOLD" 
        ? chalk.green
      ...
      const actionEmoji = pa.action === "HOLD" 
        ? "✓"
        : pa.action === "CLOSE" 
          ? "✗"
          : pa.action === "ROLL"
  ...
  if (options && options.calls && options.calls.length > 0 && totalScore >= 45) {
...
function generateConcerns(
  ...
  const { technicalScore, fundamentalScore, analystScore, upsidePotential, context, stockStyle } = score;
  ...
  if (context?.nextEarningsDate) {
    ...
    if (days <= 14 && days > 0) {
      concerns.push(`Earnings in ${days} days — consider waiting or sizing down`);
    }
  }
  ...
  if (technicalScore < 10 && !mentionedTechnicalWeak) {
    const technicalSignals = score.signals.filter(s => s.category === "technical");
  ...
  if (rsi?.status === "OVERBOUGHT") {
    concerns.push(`RSI overbought (${rsi.value.toFixed(0)}) — pullback risk elevated`);
  }

  // 52-week position concern
  if (context?.positionInRange !== undefined && context.positionInRange > 0.85) {
    concerns.push(`Trading near 52-week highs — limited upside, elevated downside`);
  }
  ...
  if (momentum) {
    ...
    if (priceMom?.direction === "deteriorating") {
      ...
      if (val < -15) {
        concerns.push(`Price down ${Math.abs(val).toFixed(0)}% (20d) — significant weakness`);
      } else if (val < -5) {
        concerns.push(`Negative price momentum (${val.toFixed(0)}% in 20 days)`);
      }
    }
    ...
    const ratingsMom = momentum.signals.find(s => s.name === "Rating Changes");
    if (ratingsMom?.direction === "deteriorating") {
      concerns.push(`More downgrades than upgrades (90 days)`);
    }
    
    // Insider selling warning
    const insiderMom = momentum.signals.find(s => s.name === "Insider Activity");
  ...
  const analystRevisions = momentum?.signals.find(s => s.name === "Analyst Revisions");
...
function displayEntryDecision(
  ...
  const aiSaysWait = !aiHasPosition && (
  ...
  if (aiHasPosition) {
    ...
    if (aiPositionAction === "HOLD" || aiPositionAction === "ADD") {
      unifiedAction = aiPositionAction === "ADD" ? "ADD TO POSITION" : "HOLD POSITION";
    ...
    unifiedColor = decision.action === "enter_now"
      ? chalk.green
      : decision.action === "wait_for_pullback"
        ? chalk.yellow
        : chalk.red;
    unifiedEmoji = decision.action === "enter_now"
      ? "✅"
      : decision.action === "wait_for_pullback"
        ? "⏳"
        : "❌";
  ...
  console.log(
    `  ${unifiedEmoji} ${chalk.bold("ACTION:")} ` +
    unifiedColor(unifiedAction)
  );
  
  // Show conflict warning if applicable
  if (hasConflict) {
    console.log();
    console.log(chalk.yellow("  ⚠ AI recommends WAIT but technicals show entry setup"));
    console.log(chalk.gray("    Consider reduced position size or wait for confirmation"));
  }
  console.log();

  // Confidence score with visual bar
  const confidenceBar = generateConfidenceBar(decision.confidence.total);
  const confidenceColor = decision.confidence.total >= 70
    ? chalk.green
  ...
  console.log(
    chalk.gray("    ├─ Stock: ") + chalk.white(`${cb.stockScore}/30`) +
    chalk.gray(" │ Checklist: ") + chalk.white(`${cb.checklistPassRate}/25`) +
    chalk.gray(" │ Momentum: ") + chalk.white(`${cb.momentum}/20`)
  );
  console.log(
    chalk.gray("    └─ Rel Strength: ") + chalk.white(`${cb.relativeStrength}/15`) +
    chalk.gray(" │ Market: ") + chalk.white(`${cb.marketRegime}/10`)
  );
  console.log();
  ...
  if (decision.recommendedSpread && decision.spreadScore) {
    ...
    const spreadRatingColor = score.rating === "excellent"
      ? chalk.green
      : score.rating === "good"
        ? chalk.cyan
        : score.rating === "fair"
    ...
    const spreadRatingStars = score.rating === "excellent"
      ? "★★★"
      : score.rating === "good"
        ? "★★"
        : score.rating === "fair"
    ...
    console.log(
      chalk.gray("  Spread: ") +
      chalk.white(`Buy $${spread.longStrike}C / Sell $${spread.shortStrike}C`) +
    ...
    console.log(
      chalk.gray("    ├─ Intrinsic: ") + chalk.white(`${sb.intrinsicValue}/20`) +
      chalk.gray(" │ Cushion: ") + chalk.white(`${sb.cushion}/20`) +
      chalk.gray(" │ Delta: ") + chalk.white(`${sb.delta}/10`)
    );
    console.log(
      chalk.gray("    ├─ DTE: ") + chalk.white(`${sb.dte}/10`) +
      chalk.gray(" │ Return: ") + chalk.white(`${sb.returnOnRisk}/10`) +
      chalk.gray(" │ Support: ") + chalk.white(`${sb.supportProtection}/15`)
    );
    console.log(
  ...
  // Position sizing
  const sizeColor = decision.positionSizing.size === "full"
    ? chalk.green
  ...
  if (decision.positionSizing.maxContracts > 0 && decision.recommendedSpread) {
    const totalCost = decision.positionSizing.maxContracts * 
                      decision.recommendedSpread.netDebit * 100;
    console.log(
...
function buildDecisionInput(
  score: StockScore,
  checklist: { passed: boolean; checks: Array<{ passed: boolean }>; failReasons: string[] },
  ...
  return {
    ...
    momentumSignals: momentum?.signals.map(s => ({
      name: s.name,
...
function evaluateSpreadChecklist(
  ...
  if (rsiValue > 70) failReasons.push(`RSI overbought (${rsiValue.toFixed(0)})`);
  
  // 3. Analyst revisions positive
  const analystRev = momentum?.signals.find(s => s.name === "Analyst Revisions");
  const revisionsPositive = analystRev?.direction === "improving" || 
    analystRev?.direction === "stable";
  checks.push({
  ...
  checks.push({
    ...
    passed: earningsOK,
    detail: daysToEarnings ? `${daysToEarnings} days to earnings` : "No earnings date"
  });
  ...
  const momNotBearish = priceMom?.direction !== "deteriorating" || momValue > -15;
  checks.push({
  ...
  const criticalPasses = aboveMA200 && rsiNotOverbought && earningsOK && scoreOK;
...
function generateDeepITMSpreads(
  ...
  const checklist = evaluateSpreadChecklist(
    currentPrice, score, rsi, momentum, daysToEarnings
  );
  ...
  for (const check of checklist.checks) {
    ...
    lines.push(`  ${icon} ${nameColor(check.name)}: ${chalk.gray(check.detail)}`);
  }
  lines.push("");
  ...
  const expiration = options.expiration instanceof Date 
    ? options.expiration 
  ...
  for (const longStrike of itmStrikes) {
    ...
    for (const shortStrike of itmStrikes) {
      ...
      if (shortItmPct < 0.02 || shortItmPct > 0.10) continue;
      ...
      if (timeValue > 0 && timeValue / netDebit > 0.50) continue;
      ...
      const deltaApprox = 0.85 - ((currentPrice - longStrike) / currentPrice) * 0.5;
      ...
      if (returnOnRisk >= 10 && breakeven < currentPrice * 0.995) {
        internalCandidates.push({
          ...
          intrinsicValue,
          timeValue,
          delta: Math.min(0.95, Math.max(0.70, deltaApprox)),
        });
      }
    }
  }
  ...
  lines.push(chalk.gray("  Buy intrinsic value, not hope | Low theta | $5 max risk per share"));
  lines.push("");
  ...
  for (const spread of topSpreads) {
    ...
    const intrinsicDisplay = intrinsicPct >= 100 
...
function getUpsideDisplay(upside: number): string {
  ...
  if (upside >= 0.30) return chalk.bold.green(`↑ ${sign}${pct}% upside`);
...
function displayResultsTableWithDecisions(
  ...
  results.slice(0, displayLimit).forEach((item, index) => {
    ...
    const scoreColor = result.totalScore >= 80 
      ? chalk.green 
...
function displayTrendsTable(
  ...
  trends.slice(0, 15).forEach((trend) => {
    const changeColor = trend.delta >= 20 
      ? chalk.green 
      : chalk.yellow;
