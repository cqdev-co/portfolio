...
export function generatePriceChart(
  ...
  for (let x = 0; x < closes.length; x++) {
    ...
    const y = Math.round(
      ((price - minPrice) / priceRange) * (cfg.height - 1)
    );
  ...
  for (let x = 0; x < ma20.length; x++) {
    const price = ma20[x];
    if (price === undefined || isNaN(price) || price < minPrice || price > maxPrice) continue;
  ...
  lines.push(chalk.gray(`  $${priceLabels[0]?.padStart(8)} ┌${"─".repeat(cfg.width)}┐`));
  ...
  for (let row = 0; row < cfg.height; row++) {
    ...
    lines.push(`${label} ${chalk.gray("│")}${grid[row]!.join("")}${chalk.gray("│")}`);
  }
  
  lines.push(chalk.gray(`  $${priceLabels[2]?.padStart(8)} └${"─".repeat(cfg.width)}┘`));
...
export function analyzeTrend(
  ...
  const fiveDayChange = recentCloses.length >= 5
    ? ((recentCloses[4] ?? 0) - (recentCloses[0] ?? 0)) / (recentCloses[0] ?? 1)
    : 0;
  
  if (fiveDayChange > 0.03) {
    bullishPoints += 1;
    signals.push(`Strong 5-day momentum (+${(fiveDayChange * 100).toFixed(1)}%)`);
...
function getTimeLabels(data: HistoricalData[], width: number): string {
  ...
  const first = firstData.date instanceof Date 
    ? firstData.date 
    : new Date(firstData.date);
  const last = lastData.date instanceof Date 
    ? lastData.date 
