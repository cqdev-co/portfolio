...
/**
 * Find the nearest strike to a target price
 */
...
/**
 * Find strikes for spread based on support/resistance levels
 */
function selectSpreadStrikes(
...
): { longStrike: number; shortStrike: number } | null {
  ...
  if (type === "bullish") {
    // Call Debit Spread: Buy ATM/slightly OTM, sell at resistance
    const longStrike = findNearestStrike(
      sortedStrikes.filter(s => s >= currentPrice * 0.98),
      currentPrice
    );
    
    const targetShort = resistance ?? currentPrice * 1.15;
    const shortStrike = findNearestStrike(
      sortedStrikes.filter(s => s > longStrike),
      targetShort
    );
    ...
    // Put Debit Spread: Buy ATM/slightly OTM, sell at support
    const longStrike = findNearestStrike(
      sortedStrikes.filter(s => s <= currentPrice * 1.02),
      currentPrice
    );
    
    const targetShort = support ?? currentPrice * 0.85;
    const shortStrike = findNearestStrike(
      sortedStrikes.filter(s => s < longStrike),
      targetShort
    );
...
export function generateSpreadRecommendation(
  ...
  const protection = support2 ?? support1 ?? currentPrice * 0.90;
  ...
  if (verdict === "buy") {
    ...
    const callShort = roundStrike(Math.min(target, currentPrice * 1.20));
