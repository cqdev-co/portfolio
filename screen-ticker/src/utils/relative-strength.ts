...
function analyzeRSLineTrend(
  ...
  const earlierAvg = earlierPeriod.reduce((a, b) => a + b, 0) / earlierPeriod.length;
...
export function calculateMultiPeriodRS(
  ...
  const outperformingCount = [rs20, rs50, rs200]
    .filter(rs => rs?.isOutperforming)
    .length;
 ...
 */
export function formatRelativeStrength(
