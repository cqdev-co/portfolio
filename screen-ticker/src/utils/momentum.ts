...
function analyzePriceMomentum(
  ...
  if (roc20 > 5) {
    direction = 'improving';
    description =
      `20-day: +${roc20.toFixed(1)}%` +
      (roc50 !== null
        ? `, 50-day: ${roc50 > 0 ? '+' : ''}${roc50.toFixed(1)}%`
        : '');
  } else if (roc20 < -5) {
    direction = 'deteriorating';
    description =
...
function analyzeEPSTrend(summary: QuoteSummary): MomentumSignal | null {
  ...
  const change30d =
    thirtyDaysAgo !== 0
      ...
      : 0;
  const change90d =
    ninetyDaysAgo !== undefined && ninetyDaysAgo !== 0
...
function analyzeEPSRevisions(summary: QuoteSummary): MomentumSignal | null {
  ...
  const { upLast30days, downLast30days } = currentQ.epsRevisions;
 ...
 */
function analyzeInstitutionalOwnership(
  ...
  const direction: MomentumDirection = 'stable';
  let description: string;
...
function analyzeProfitabilityTrend(
  ...
  if (currentYearGrowth !== undefined && currentYearGrowth > 0.5) {
  ...
  } else if (currentYearGrowth !== undefined && currentYearGrowth < -0.2) {
    // Deteriorating
    direction = 'deteriorating';
...
function analyzeQuarterlyTrend(summary: QuoteSummary): MomentumSignal | null {
  ...
  const oldestRev = revenues[0] ?? 0;
  const newestRev = revenues[revenues.length - 1] ?? 0;
  const revGrowth =
    oldestRev !== 0 ? ((newestRev - oldestRev) / Math.abs(oldestRev)) * 100 : 0;

  // Check earnings trend
  const oldestEarn = earnings[0] ?? 0;
  const newestEarn = earnings[earnings.length - 1] ?? 0;
  const earnGrowth =
    oldestEarn !== 0
      ? ((newestEarn - oldestEarn) / Math.abs(oldestEarn)) * 100
      : 0;
