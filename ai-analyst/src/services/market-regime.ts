...
function generateRecommendation(
  ...
  switch (regime) {
    case 'RISK_ON':
      return 'Favorable for CDS entries. Normal position sizing. Look for pullbacks to MA20.';
    case 'RISK_OFF':
      return 'Reduce exposure. Wait for trend reversal signals. Consider defensive sectors.';
    case 'HIGH_VOL':
      if (vix && vix.level === 'EXTREME') {
        return 'Avoid new entries. Wait for VIX to drop below 30 before trading.';
      }
      return 'Reduce position sizes by 50%. Wider stops required. Consider waiting.';
    case 'NEUTRAL':
      return 'Proceed with caution. Focus on Grade A setups only. Normal position sizing.';
  }
}
