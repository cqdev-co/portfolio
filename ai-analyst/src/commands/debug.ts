    ...
    try {
      ...
      if (volHistory?.quotes && volHistory.quotes.length >= 10) {
        ...
        if (vols.length >= 10) {
          ...
          unusualDays = vols.slice(-10).filter(v => v > overallAvg * 2).length;
        }
      }
    } catch {
...
export async function debugCommand(
  ...
  const [stockData, marketData, pfvData] = 
    await Promise.all([
  ...
  const analysisInput: AnalysisDataInput = {
    stock: {
      ...
      riskReward: stockData.spread ? {
        ...
        profitPct: Math.round((stockData.spread.maxProfit / stockData.spread.estimatedDebit) * 100),
  ...
  log.log(chalk.gray(`  System Prompt: ~${systemTokens} tokens ` +
    `(${systemPrompt.length} chars)`));
  log.log(chalk.gray(`  User Prompt:   ~${userTokens} tokens ` +
  ...
  log.log(chalk.gray(`    Input:  $${inputCost.toFixed(6)} ` +
    `($0.14/M tokens)`));
  log.log(chalk.gray(`    Output: $${outputCost.toFixed(6)} ` +
    `($0.28/M tokens)`));
  ...
  log.log(chalk.gray(`    JSON:    ${jsonChars} chars → ` +
    `~${Math.ceil(jsonChars / 3.5)} tokens`));
  log.log(chalk.gray(`    TOON:    ${toonChars} chars → ` +
