...
async function fetchStockData(ticker: string): Promise<StockData | null> {
  try {
    ...
    if (recTrend) {
      ...
      const upgradeModule = insightsAny?.upgradeDowngradeHistory as
        | {
            history?: Array<{
        ...
        | undefined;
      const history = upgradeModule?.history ?? [];
    ...
    const earningsHistModule = insightsAny?.earningsHistory as
      | {
          history?: Array<{
            epsActual?: number;
            epsEstimate?: number;
      ...
      | undefined;
    const earningsHist = earningsHistModule?.history ?? [];
...
async function generateAIAnalysis(
  ...
  const { buildAnalysisSystemPrompt, buildAnalysisUserPrompt } =
      ...
      riskReward: stock.spread
        ? {
            ...
            profitPct: Math.round(
              (stock.spread.maxProfit / stock.spread.estimatedDebit) * 100
            ),
...
function displayHeader(
  ...
  const scoreDisplay = technical?.score
    ? technical.score >= 70
      ? chalk.green(`${technical.score}/100 ★★`)
      : technical.score >= 50
        ? chalk.yellow(`${technical.score}/100 ★`)
...
function displayPsychologicalFairValue(
  pfv: PsychologicalFairValue | null
): void {
  console.log();
  ...
  console.log(
    `  Fair Value: ${chalk.bold.white('$' + pfv.fairValue.toFixed(2))}  ` +
      ...
      biasColor(
        `${pfv.deviationPercent > 0 ? '+' : ''}${pfv.deviationPercent.toFixed(1)}% ${pfv.bias}`
      )
  );
  console.log(
  ...
  if (pfv.supportZone || pfv.resistanceZone) {
    if (pfv.supportZone) {
      console.log(
        chalk.green(
          `  Support Zone: $${pfv.supportZone.low.toFixed(2)} - $${pfv.supportZone.high.toFixed(2)}`
        )
      );
    }
    if (pfv.resistanceZone) {
      console.log(
        chalk.red(
          `  Resistance Zone: $${pfv.resistanceZone.low.toFixed(2)} - $${pfv.resistanceZone.high.toFixed(2)}`
        )
      );
    }
    console.log();
...
function formatAIOutput(text: string): string {
  ...
  for (const section of sections) {
    ...
    if (sectionMatch) {
      ...
      lines.push('');
      lines.push(
        `  ${icon} ${chalk.bold.cyan(num + '. ' + title.trim().toUpperCase())}`
      );
      lines.push('');
...
function displayEntryDecision(
  ...
  if (!aboveMA200) {
    ...
    action = 'WAIT';
    reasons.push(
      `RSI elevated (${technical?.rsi?.toFixed(0)}) - wait for pullback`
    );
  } else if (!valueOK) {
    action = 'PASS';
    ...
    if (
      ...
      reasons.push('Partial bullish signals - smaller position advised');
    } else {
      action = 'ENTER';
  ...
  if (pfv) {
    if (
      pfv.primaryExpiration?.isMonthlyOpex &&
      pfv.primaryExpiration.dte <= 7
    ) {
      reasons.push(
        `⚠️ Monthly OPEX in ${pfv.primaryExpiration.dte} days - max pain magnetism active`
      );
    }
  }
  ...
  console.log(
    `  ${actionEmoji} ${chalk.bold('ACTION:')} ${actionColor(action.replace('_', ' '))}`
  );
  console.log();
  ...
  console.log(
    chalk.gray(
      `  Position Size: ${strategy.positionSize.toFixed(0)}% of account ($${strategy.riskAmount.toFixed(0)} risk)`
    )
  );
  console.log();
}
...
export async function analyzeCommand(
  ...
  console.log(
    chalk.gray(
      `  AI mode: ${options.aiMode}${options.aiModel ? ` (${options.aiModel})` : ''}`
    )
  );
  console.log();
  ...
  displayAIAnalysis(aiAnalysis, options.aiModel ?? '', options.aiMode);
