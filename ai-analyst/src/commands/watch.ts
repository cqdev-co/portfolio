...
export async function listWatchlist(): Promise<void> {
  ...
  if (watchlist.length === 0) {
    ...
    console.log(
      chalk.gray(
        '  Add tickers with: bun run analyst watch add NVDA AAPL GOOGL'
      )
    );
    console.log();
    return;
...
export async function addToWatch(
  ...
  if (tickers.length === 0) {
    ...
    console.log();
    console.log(
      chalk.gray('  Usage: bun run analyst watch add NVDA AAPL GOOGL')
    );
    console.log();
    return;
  }
...
export async function removeFromWatch(ticker: string): Promise<void> {
  ...
  if (success) {
    console.log(
      chalk.green(`  ✓ Removed ${ticker.toUpperCase()} from watchlist`)
    );
  } else {
...
export async function configureWatch(
  ...
  if (!hasOptions) {
    ...
    console.log(
      chalk.gray('    --rsi-low <n>       Minimum RSI threshold (default: 35)')
    );
    console.log(
      chalk.gray('    --rsi-high <n>      Maximum RSI threshold (default: 55)')
    );
    console.log(
      chalk.gray(
        '    --iv <n>            IV percentile threshold (default: 50)'
      )
    );
    console.log(
      chalk.gray('    --cushion <n>       Minimum cushion % (default: 8)')
    );
    console.log(
      chalk.gray('    --grade <g>         Minimum grade (default: B)')
    );
    console.log(chalk.gray('    --notes <text>      Notes about this ticker'));
    console.log(
      chalk.gray('    --active/--inactive Enable/disable monitoring')
    );
    console.log();
    return;
  }
  ...
  if (result) {
    ...
    console.log(
      chalk.gray(
        `    RSI Range: ${result.targetRsiLow}-${result.targetRsiHigh}`
      )
    );
    ...
    console.log(
      chalk.gray('    Make sure the ticker is in your watchlist first.')
    );
  }
