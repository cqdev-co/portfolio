...
async function fetchTechnicals(
  ...
  try {
    ...
    const quotes = history.quotes.filter(
      q => q.close !== null && q.high !== null && 
           q.low !== null && q.volume !== null
    );
    ...
    const typicalPrices = quotes.map((q, i) => 
 ...
 */
function getNextFridayExpiration(): { date: Date; formatted: string; dte: number } {
  const now = new Date();
...
function generateEntrySignal(
  ...
  if (technicals.rsiSignal === "oversold") {
    score += 25;
    reasons.push(`RSI oversold at ${technicals.rsi.toFixed(0)} - bounce likely`);
  } else if (technicals.rsiSignal === "overbought") {
    score -= 20;
    warnings.push(`RSI overbought at ${technicals.rsi.toFixed(0)} - risky entry`);
  }
  ...
  } else if (technicals.vwapDeviation > Math.abs(ENTRY_THRESHOLDS.vwapDeviation)) {
    score -= 10;
    warnings.push(
  ...
  if (signal === "STRONG_BUY" || signal === "BUY") {
    ...
    const longStrike = Math.floor(currentPrice / spreadWidth) * spreadWidth - spreadWidth;
    const shortStrike = longStrike + spreadWidth;
    
    // Estimate cost (typically 60-75% of spread width for ITM)
    const estimatedCost = spreadWidth * 0.70 * 100;  // Per contract
...
function printMarketStatus(
  ...
  const vixColor = vix.level === "extreme" ? chalk.red :
                   vix.level === "elevated" ? chalk.yellow :
                   chalk.green;
...
function printIndexCard(
  ...
  // Technicals
  const rsiColor = technicals.rsiSignal === "oversold" ? chalk.green :
                   technicals.rsiSignal === "overbought" ? chalk.red :
                   chalk.white;
  const trendIcon = technicals.trend === "bullish" ? "📈" :
                    technicals.trend === "bearish" ? "📉" : "➡️";
...
export async function shortTermCommand(
  ...
  const spySignal = generateEntrySignal(
    "SPY", spyData, spyTech, vix, options.accountSize
  );
  const qqqSignal = generateEntrySignal(
    "QQQ", qqqData, qqqTech, vix, options.accountSize
  );
