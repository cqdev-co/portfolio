...
async function displayTickerJournal(ticker: string): Promise<void> {
  ...
  for (const trade of trades.slice(0, 10)) {
    ...
    table.push([dateStr, typeStr, strikes, daysStr, outcomeStr, pnlStr]);
  }
...
async function displayPerformanceStats(): Promise<void> {
  ...
  console.log(
    `  Open: ${chalk.yellow(summary.openTrades.toString())} | Closed: ${summary.closedTrades}`
  );
  console.log(
...
export async function logTrade(options: LogTradeOptions): Promise<void> {
  ...
  try {
    ...
    if (trade) {
      ...
      console.log(
        chalk.gray(
          `    ${trade.ticker} ${formatTradeType(trade.tradeType)} ${trade.longStrike}/${trade.shortStrike}`
        )
      );
      console.log(
        chalk.gray(
          `    Premium: $${(trade.openPremium * 100).toFixed(0)} | Expires: ${trade.expiration.toLocaleDateString()}`
        )
      );
      console.log();
    } else {
