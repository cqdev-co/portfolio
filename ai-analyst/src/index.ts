...
program
  ...
  .description("Analyze a ticker for entry decision and strategy recommendation")
  .alias("a")
  .option(
    "--ai-mode <mode>", 
  ...
  .option(
    "-p, --position <spread>", 
    "Your position (e.g., '120/125 CDS')"
  )
  .option(
    "--no-chart", 
    "Skip price chart visualization"
  )
  .option(
    "-a, --account <size>", 
...
agentCommand
  ...
  .action(async () => {
    ...
    if (success) {
      ...
      console.log(chalk.gray("  Check your Discord channel for the test message."));
    } else {
      console.log(chalk.red("  ✗ Discord webhook test failed."));
      console.log(chalk.gray("  Make sure DISCORD_WEBHOOK_URL is set in your .env file."));
    }
    console.log();
  });
...
program.on("--help", () => {
  ...
  console.log();
  console.log("  $ bun run briefing                 # Generate morning briefing");
  ...
  console.log();
  console.log("  $ bun run agent start              # Start background monitoring");
  console.log("  $ bun run agent start --debug      # Start with rejection logging");
  console.log("  $ bun run agent start --extended-hours  # Scan 24/7");
  console.log("  $ bun run agent dry-run            # Single scan with full debug output");
  console.log("  $ bun run agent stop               # Stop background monitoring");
  ...
  console.log("  $ bun run alerts --unack           # View unacknowledged alerts");
