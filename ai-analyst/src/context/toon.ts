  ...
  return {
    ...
    volume: stock.volumeAnalysis
      ? {
          ...
          unusual:
            stock.volumeAnalysis.unusualDays > 0
    ...
    volatility: stock.volatility
      ? {
          ...
          hv20: stock.volatility.hv20
            ? `${stock.volatility.hv20.toFixed(1)}%`
            : null,
    ...
    optionsFlow: stock.optionsFlow
      ? {
          ...
          sentiment:
            stock.optionsFlow.pcRatioOI < 0.7
              ? 'bullish'
              : stock.optionsFlow.pcRatioOI > 1.0
                ? 'bearish'
                : 'neutral',
        }
      : null,
...
export function buildTOONContext(
  ...
  for (const [ticker, trades] of tickerHistories) {
    if (trades.length > 0) {
      tickers.push(compressTickerHistory(ticker, trades, patterns.get(ticker)));
    }
  }
...
export function encodeTickerToTOON(data: TickerDataInput): string {
  ...
  if (data.iv || data.hv20) {
    ...
    if (iv && hv) {
      premium =
        iv > hv * 1.15 ? 'expensive' : iv < hv * 0.85 ? 'cheap' : 'fair';
    }
  ...
  if (data.nearestSupport || data.nearestResistance) {
    tickerData.levels = {
      ...
      resistance: data.nearestResistance
        ? Math.round(data.nearestResistance)
        : null,
    };
  }
  ...
  if (data.targetLow && data.targetMean && data.targetHigh) {
    tickerData.targets = {
      ...
      upside: data.targetUpside
        ? `${data.targetUpside > 0 ? '+' : ''}${data.targetUpside}%`
        : null,
    };
  }
  ...
  if (data.perf5d !== undefined || data.perf1m !== undefined) {
    tickerData.perf = {
      d5:
        data.perf5d !== undefined
          ? `${data.perf5d > 0 ? '+' : ''}${data.perf5d}%`
          : null,
      m1:
        data.perf1m !== undefined
          ? `${data.perf1m > 0 ? '+' : ''}${data.perf1m}%`
          : null,
      m3:
        data.perf3m !== undefined
          ? `${data.perf3m > 0 ? '+' : ''}${data.perf3m}%`
          : null,
      ytd:
        data.perfYtd !== undefined
          ? `${data.perfYtd > 0 ? '+' : ''}${data.perfYtd}%`
          : null,
    };
  }
  ...
  if (data.earningsDate || data.earningsStreak !== undefined) {
    tickerData.earn = {
      ...
      streak: data.earningsStreak
        ...
        : null,
      surprise:
        data.earningsSurprise !== undefined
...
function extractMessageKeyInfo(msg: ConversationMessage): {
  ...
  const tickers = [
    ...new Set(tickerMatches.filter((t) => !commonWords.has(t))),
  ];
...
export function summarizeConversation(
  ...
  return summaries.length > 0 ? `Prior: ${summaries.join(' ')}` : '';
}
...
export function detectPatterns(trades: Trade[]): string[] {
  ...
  if (tradesWithRsi.length >= 3) {
    ...
    if (winningRsis.length > 0 && losingRsis.length > 0) {
      const avgWinRsi =
        winningRsis.reduce((a, b) => a + b, 0) / winningRsis.length;
      const avgLossRsi =
        losingRsis.reduce((a, b) => a + b, 0) / losingRsis.length;

      if (avgLossRsi > avgWinRsi + 10) {
        patterns.push(
          `Losses tend to occur at RSI > ${Math.round(avgLossRsi)}`
        );
      }
  ...
  if (tradesWithDays.length >= 3) {
    ...
    if (losingDays.length > 0) {
      const avgLossDays =
        losingDays.reduce((a, b) => a + b, 0) / losingDays.length;
      if (avgLossDays > 14) {
        patterns.push(
          `Consider cutting losses earlier (avg loss held ${Math.round(avgLossDays)} days)`
        );
      }
    }
  }
