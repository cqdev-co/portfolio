...
export interface MarketContext {
  ...
  vix: number;
  vixLevel: 'CALM' | 'NORMAL' | 'ELEVATED' | 'HIGH' | 'EXTREME';
}
...
export function calculateDynamicAdjustments(
  ...
  if (context.vixLevel === 'HIGH' || context.vix > 25) {
    ...
    adjustments.reasons.push(
      `Elevated VIX (${context.vix.toFixed(0)}): IV +8%`
    );
  } else if (context.vixLevel === 'CALM' || context.vix < 15) {
  ...
  // 4. High Volatility Regime: Relax cushion slightly
  if (context.regime === 'HIGH_VOL') {
    adjustments.cushionAdjust -= 1;
    adjustments.reasons.push('High volatility regime: cushion -1%');
  }
  ...
  if (!entryConditionsMet) {
    ...
    if (!criteria.rsiInBuyZone) {
      rejectionReasons.push(
        `RSI outside range (${criteria.rsiLow}-${criteria.rsiHigh})`
      );
    }
    if (!criteria.ivBelowThreshold) {
      rejectionReasons.push(
        `IV too high (${criteria.ivPercentile}% > ${criteria.ivThreshold}%)`
      );
    }
    ...
    if (criteria.earningsWithin7Days)
      rejectionReasons.push('Earnings within 7 days');
...
function buildCriteria(
  ...
  const effectiveRsiLow = Math.max(
    15,
    watchlistItem.targetRsiLow + adjustments.rsiLowAdjust
  );
  const effectiveRsiHigh = Math.min(
    80,
    watchlistItem.targetRsiHigh + adjustments.rsiHighAdjust
  );
  const effectiveIvThreshold = watchlistItem.ivPercentileMin
    ? Math.min(
        100,
        Math.max(0, watchlistItem.ivPercentileMin + adjustments.ivAdjust)
      )
    : undefined;
  const effectiveCushion = Math.max(
    3,
    watchlistItem.minCushionPct + adjustments.cushionAdjust
  );
  ...
  return {
    ...
    rsiInBuyZone:
      rsi !== undefined && rsi >= effectiveRsiLow && rsi <= effectiveRsiHigh,
    ...
    cushionAboveMin: cushion !== undefined && cushion >= effectiveCushion,

    gradeMetMin:
      gradeToValue(result.grade.grade) >= gradeToValue(watchlistItem.minGrade),
    ...
    // Risk flags
    earningsWithin7Days:
      earningsDays !== undefined && earningsDays !== null && earningsDays <= 7,
