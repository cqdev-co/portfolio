 ...
 */
export function scoreStructure(response: string, context: ResponseContext): QualityDimension {
  ...
  const actionPatterns = [
    /recommend/i, /suggest/i, /consider/i, /would/i,
    /buy/i, /sell/i, /hold/i, /close/i, /roll/i,
  ];
  ...
  const reasoningPatterns = [
    /because/i, /since/i, /given/i, /considering/i,
    /due to/i, /based on/i, /therefore/i,
  ];
  ...
  const dataPatterns = [
    /RSI/i, /MA/i, /IV/i, /support/i, /resistance/i,
    /volume/i, /earnings/i, /P\/E/i, /cushion/i,
  ];
 ...
 */
export function scoreReasoning(response: string, context: ResponseContext): QualityDimension {
  const feedback: string[] = [];
  let score = 0;
  const maxScore = 30;
  ...
  const causalPatterns = [
    /if .* then/i, /when .* will/i, /because/i, /therefore/i,
    /this means/i, /which indicates/i, /suggests that/i,
    /since/i, /given that/i, /so /i, /thus/i,
  ];
  ...
  const factorPatterns = [
    /first/i, /second/i, /additionally/i, /also/i,
    /moreover/i, /furthermore/i, /another/i, /on the other hand/i,
  ...
  const quantPatterns = [
    /\d+%/, /\d+\.\d+/, /\$\d+/, /\d+ days/i,
    /ratio/i, /level/i, /target/i, /at \d/i,
  ];
  ...
  const scenarioPatterns = [
    /if .* breaks/i, /should .* hold/i, /worst case/i,
    /best case/i, /scenario/i, /alternative/i, /otherwise/i,
    /if .* drops/i, /if .* fails/i, /wait for/i,
  ];
  ...
  const timePatterns = [
    /short.?term/i, /long.?term/i, /near.?term/i,
    /days/i, /weeks/i, /expir/i, /DTE/i, /by .* date/i,
    /next week/i, /earnings/i, /patience/i,
  ];
 ...
 */
export function scoreRiskManagement(response: string, context: ResponseContext): QualityDimension {
  const feedback: string[] = [];
  let score = 0;
  const maxScore = 25;
  ...
  const riskPatterns = [
    /risk/i, /downside/i, /caution/i, /careful/i,
    /warning/i, /danger/i, /concern/i, /watch out/i,
  ];
  ...
  const exitPatterns = [
    /stop/i, /exit/i, /close.*if/i, /cut/i,
    /protect/i, /limit.*loss/i, /max.*loss/i,
  ];
  ...
  const sizingPatterns = [
    /position size/i, /allocation/i, /% of/i,
    /contract/i, /not too (much|large)/i, /small/i,
  ];
  ...
  const bufferPatterns = [
    /cushion/i, /buffer/i, /margin/i, /room/i,
    /above/i, /below/i, /distance/i,
  ];
  ...
  const breakevenPatterns = [
    /breakeven/i, /break.?even/i, /B\/E/i,
    /profitable above/i, /profitable below/i,
  ];
 ...
 */
export function scoreActionability(response: string, context: ResponseContext): QualityDimension {
  const feedback: string[] = [];
  let score = 0;
  const maxScore = 20;
  ...
  const actionVerbs = [
    /\b(buy|sell|hold|close|roll|wait|enter|exit)\b/i,
  ];
  ...
  const timingPatterns = [
    /now/i, /wait/i, /before/i, /after/i,
    /today/i, /this week/i, /expiration/i,
  ];
  ...
  const confidencePatterns = [
    /confident/i, /likely/i, /probably/i, /strong/i,
    /weak/i, /uncertain/i, /50\/50/i, /high probability/i,
  ];
...
export function detectHallucinations(
  ...
  const wrongTickers = mentionedTickers.filter(t => 
    t !== providedData.ticker && 
    !['RSI', 'MA', 'IV', 'HV', 'DTE', 'ITM', 'OTM', 'ATM', 'P/E', 'EPS'].includes(t)
  );
  ...
  if (providedData.price) {
    ...
    const mentionedPrices = [...response.matchAll(pricePattern)].map(m => parseFloat(m[1]));
    const unrealistic = mentionedPrices.filter(p => 
      p > providedData.price * 2 || p < providedData.price * 0.5
    );
    if (unrealistic.length > 0) {
      issues.push(`Potentially unrealistic prices: $${unrealistic.join(', $')}`);
    }
  }
...
export function validatePositionResponse(
  ...
  if (toolWasUsed && toolOutput) {
    ...
    const profitCapturedMatch = toolOutput.match(/Profit Captured:\s*([\d.]+)%/);
    if (profitCapturedMatch) {
      const toolValue = parseFloat(profitCapturedMatch[1]);
      // Check if response mentions a very different number
      const responseNumbers = response.match(/(\d{1,3}(?:\.\d)?)\s*%.*(?:captured|profit)/gi);
      if (responseNumbers) {
        for (const match of responseNumbers) {
          const num = parseFloat(match);
          if (Math.abs(num - toolValue) > 10) {
            issues.push(`Response profit % (${num}%) differs from tool output (${toolValue}%)`);
          }
        }
      }
    }
