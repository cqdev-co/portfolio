...
export type RecommendationType = "BULLISH" | "BEARISH" | "NEUTRAL" | "HOLD" | "CLOSE";
export type SpreadType = "CALL_DEBIT" | "PUT_DEBIT" | "CALL_CREDIT" | "PUT_CREDIT" | "IRON_CONDOR";
export type OutcomeStatus = "PENDING" | "WIN" | "LOSS" | "BREAKEVEN" | "EXPIRED";
...
export interface PerformanceMetrics {
  ...
  byRecommendationType: Record<RecommendationType, { wins: number; losses: number }>;
...
export function loadRecords(dataDir: string = DEFAULT_DATA_DIR): BacktestRecord[] {
...
export function saveRecords(records: BacktestRecord[], dataDir: string = DEFAULT_DATA_DIR): void {
  ensureDataDir(dataDir);
  const path = getRecordsPath(dataDir);
...
export function getRecommendation(id: string, dataDir: string = DEFAULT_DATA_DIR): BacktestRecord | undefined {
  const records = loadRecords(dataDir);
  return records.find(r => r.recommendation.id === id);
}

export function getPendingRecommendations(dataDir: string = DEFAULT_DATA_DIR): BacktestRecord[] {
  const records = loadRecords(dataDir);
...
export function recordOutcome(
  ...
  const records = loadRecords(dataDir);
  const index = records.findIndex(r => r.recommendation.id === recommendationId);
  ...
  const daysHeld = outcome.closedAt 
    ? Math.ceil((new Date(outcome.closedAt).getTime() - new Date(rec.timestamp).getTime()) / (1000 * 60 * 60 * 24))
    : undefined;
  ...
  if (!spread) {
    ...
    if (rec.targetPrice && currentPrice >= rec.targetPrice) {
      return {
        ...
        actualProfitPct: ((currentPrice - rec.priceAtRecommendation) / rec.priceAtRecommendation) * 100,
      };
    }
    ...
    if (isExpired) {
      if (currentPrice >= spread.shortStrike) {
        ...
        return {
          ...
          actualProfitPct: (spread.estimatedMaxProfit / spread.estimatedDebit) * 100,
        };
...
export function calculateMetrics(dataDir: string = DEFAULT_DATA_DIR): PerformanceMetrics {
  const records = loadRecords(dataDir);
...
export function formatMetricsReport(metrics: PerformanceMetrics): string {
  ...
  // By Ticker
  const tickers = Object.entries(metrics.byTicker)
  ...
  // Monthly
  const months = Object.entries(metrics.monthlyPerformance)
 ...
 */
export function calculateSharpeRatio(dataDir: string = DEFAULT_DATA_DIR): number {
  const records = loadRecords(dataDir);
