...
function calculateSpreadMetrics(params: SpreadParams): SpreadMetrics {
  const { currentPrice, longStrike, shortStrike, estimatedDebit, iv, dte } = params;
...
describe("Spread Metrics Calculations", () => {
  ...
  describe("Basic Spread Math", () => {
    ...
    test("$10 wide spread", () => {
      ...
      expect(metrics.breakeven).toBe(348.50);
  ...
  describe("Return on Risk", () => {
    ...
    test("High return spread (25%+)", () => {
      const metrics = calculateSpreadMetrics({
        ...
        longStrike: 95,
        shortStrike: 100,
        estimatedDebit: 4.00,  // 20% max profit
        iv: 0.30,
        dte: 30,
      });
    ...
    test("Low return spread (<15%)", () => {
      const metrics = calculateSpreadMetrics({
        ...
        longStrike: 90,
        shortStrike: 95,
        estimatedDebit: 4.50,  // Only $0.50 max profit
        iv: 0.30,
        dte: 30,
      });
...
describe("Probability of Profit (PoP)", () => {
  ...
  describe("IV Impact on PoP", () => {
    ...
    test("Higher IV = lower PoP (more uncertainty)", () => {
      const popLowIV = calculatePoP(190, 185, 0.20, 30);
      const popHighIV = calculatePoP(190, 185, 0.50, 30);
  ...
  describe("PoP Edge Cases", () => {
    ...
    test("PoP capped at 95%", () => {
      const pop = calculatePoP(300, 180, 0.10, 1);  // Way above breakeven, low IV, short DTE
      expect(pop).toBeLessThanOrEqual(95);
    });
    
    test("PoP floored at 5%", () => {
      const pop = calculatePoP(150, 200, 0.10, 1);  // Way below breakeven
...
describe("Real-World Spread Scenarios", () => {
  ...
  test("AVGO spread from chat conversation", () => {
    ...
    expect(metrics.spreadWidth).toBe(5);
    expect(metrics.maxProfit).toBe(1.70);
    expect(metrics.breakeven).toBe(343.30);
...
describe("Risk/Reward Validation", () => {
  ...
  test("Minimum acceptable R/R (10%+)", () => {
    const metrics = calculateSpreadMetrics({
      ...
      longStrike: 95,
      shortStrike: 100,
      estimatedDebit: 4.50,  // 11.1% return ($0.50 / $4.50)
      iv: 0.30,
      dte: 30,
    });
  ...
  test("Reject spreads with <10% R/R", () => {
    const metrics = calculateSpreadMetrics({
      ...
      longStrike: 95,
      shortStrike: 100,
      estimatedDebit: 4.60,  // Only ~8.7% return
      iv: 0.30,
      dte: 30,
    });
