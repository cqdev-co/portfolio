...
await test('GET /options/AAPL returns response', async () => {
  ...
  const hasAuthError =
    data.finance?.error?.code === 'Unauthorized' || data.optionChain?.error;
  if (hasData) {
    ...
    console.log(colors.dim(`    ⚠ Yahoo requires auth - options limited`));
  }
...
await test('GET /summary/AAPL returns response', async () => {
  ...
  const hasAuthError =
    data.finance?.error?.code === 'Unauthorized' || data.quoteSummary?.error;
  if (hasData) {
    console.log(colors.dim('    Full summary data available'));
  } else if (hasAuthError) {
    console.log(colors.dim(`    ⚠ Yahoo requires auth - summary limited`));
  }
  assert(hasData || hasAuthError, 'Should return data or auth error');
});
