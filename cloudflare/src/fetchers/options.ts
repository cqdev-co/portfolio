...
export async function fetchOptions(
  ...
  if (cached) {
    logger.debug(`[Cache] HIT options/${ticker} (age: ${cached.age}s)`);
    return cached.data;
  }
  ...
  const data = await withRetry(
    () =>
      fetchYahooAPI<YahooOptionsResponse>(
        `https://query1.finance.yahoo.com/v7/finance/options/${encodedTicker}`,
        auth
      ),
    `options/${ticker}`
  );
  ...
  const result: OptionsData = {
    ...
    putOI,
    pcRatioVol:
      callVolume > 0 ? Math.round((putVolume / callVolume) * 100) / 100 : null,
    pcRatioOI: callOI > 0 ? Math.round((putOI / callOI) * 100) / 100 : null,
  };
