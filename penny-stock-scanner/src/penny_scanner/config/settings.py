...
class Settings(BaseSettings):
    ...
    penny_min_price: float = Field(
        default=0.10,
        description="Minimum penny stock price"
    )
    penny_max_price: float = Field(
        default=5.00,
        description="Maximum penny stock price"
    )
    penny_min_volume: int = Field(
        default=200000,
    ...
    volume_spike_2x: float = Field(
        default=2.0,
        description="2x volume spike threshold"
    )
    volume_spike_3x: float = Field(
        default=3.0,
        description="3x volume spike threshold"
    )
    volume_spike_5x: float = Field(
        default=5.0,
        description="5x volume spike threshold"
    )
    ...
    weight_volume_surge: float = Field(
        default=0.20,
        description="Weight for volume surge (20% of total)"
    )
    weight_volume_acceleration: float = Field(
        default=0.15,
        description="Weight for volume acceleration (15% of total)"
    )
    weight_volume_consistency: float = Field(
        default=0.10,
        description="Weight for volume consistency (10% of total)"
    )
    weight_liquidity_depth: float = Field(
        default=0.05,
        description="Weight for liquidity depth (5% of total)"
    )
    
    # Price Momentum & Consolidation: 30%
    weight_consolidation: float = Field(
        default=0.12,
        description="Weight for consolidation detection (12% of total)"
    )
    weight_price_acceleration: float = Field(
        default=0.10,
        description="Weight for price acceleration (10% of total)"
    )
    weight_higher_lows: float = Field(
        default=0.05,
        description="Weight for higher lows pattern (5% of total)"
    )
    ...
    weight_bid_ask_spread: float = Field(
        default=0.02,
        description="Weight for bid-ask spread (2% of total)"
    )
    weight_float_analysis: float = Field(
        default=0.02,
        description="Weight for float analysis (2% of total)"
    )
    weight_price_stability: float = Field(
        default=0.01,
        description="Weight for price stability (1% of total)"
    )
    ...
    volume_sma_period: int = Field(
        default=20, description="Volume SMA period"
    )
    ...
    default_profit_target_pct: float = Field(
        default=0.25,
        description="Default profit target percentage"
    )
    ...
    analysis_concurrency: int = Field(
        default=10,
        description="Number of concurrent analysis workers"
    )
