...
from penny_scanner.models.market_data import (
    MarketData,
    OHLCVData,
    TechnicalIndicators
)
...
class TechnicalIndicatorCalculator:
    ...
    def _calculate_emas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['ema_20'] = df['close'].ewm(
            span=self.settings.ema_short_period, 
            adjust=False
        ).mean()
        df['ema_50'] = df['close'].ewm(
            span=self.settings.ema_long_period,
            adjust=False
        ).mean()
        df['sma_20'] = df['close'].rolling(
            window=self.settings.ema_short_period
        ).mean()
        return df
    ...
    def _calculate_atr(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ATR
        df['atr_20'] = df['true_range'].rolling(
    ...
    def calculate_volume_acceleration(
        ...
        for period in periods:
            ...
            previous_volumes = [
                d.volume for d in ohlcv_data[-period*2:-period]
            ]
    ...
    def count_consecutive_green_days(
        ...
        for i in range(len(ohlcv_data) - 1, 
                      max(0, len(ohlcv_data) - max_lookback - 1), -1):
    ...
    def calculate_volume_consistency(
        ...
        high_volume_days = sum(
            ...
            if d.volume >= baseline_avg * threshold_multiplier
        )
