...
class PerformanceTrackingService:
    ...
    async def track_new_signals(self, signals: List[AnalysisResult], scan_date: date = None) -> int:
        ...
        for signal in signals:
            try:
                ...
                response = self.database_service.client.table('penny_signal_performance').insert(
                    performance_data
                ).execute()
    ...
    async def close_ended_signals(self, ended_symbols: List[str], scan_date: date = None) -> int:
        ...
        for symbol in ended_symbols:
            try:
                ...
                perf_response = self.database_service.client.table('penny_signal_performance').select(
                    '*'
                ...
                if stop_loss_price and self.data_service:
                    try:
                        ...
                        checker = StopLossChecker(self.database_service, self.data_service)
                        ...
                        if stop_result['stop_hit']:
                            ...
                            logger.info(f"{symbol}: Stopped out at ${exit_price:.2f} on {exit_date}")
                    except Exception as e:
    ...
    async def _get_signal_id(self, symbol: str, scan_date: date) -> Optional[str]:
        ...
        try:
            response = self.database_service.client.table('penny_stock_signals').select(
                'id'
            ).eq('symbol', symbol).eq('scan_date', scan_date.isoformat()).limit(1).execute()
        ...
        try:
            ...
            for i, signal in enumerate(signals):
                ...
                if (current_episode is None or 
                    current_episode['symbol'] != symbol or 
                    (scan_date - current_episode['last_date']).days > 3):
            ...
            logger.info(f"Identified {len(episodes)} signal episodes. Calculating performance...")
            ...
            for episode in episodes:
                ...
                existing = self.database_service.client.table('penny_signal_performance').select(
                    'id'
                ).eq('signal_id', start_signal['id']).execute()
                ...
                status = 'CLOSED'
                exit_date = last_date  # Default to last seen date if we can't get next day
                ...
                # Insert
                self.database_service.client.table('penny_signal_performance').insert(perf_data).execute()
                backfilled_count += 1
            
            logger.info(f"Successfully backfilled {backfilled_count} performance records")
    ...
    async def get_performance_by_rank(self) -> Dict[str, Dict]:
        ...
        try:
            response = self.database_service.client.table('penny_signal_performance').select(
                'opportunity_rank, return_pct, is_winner'
    ...
    async def get_top_trades(self, limit: int = 5) -> Dict[str, List]:
        ...
        try:
            # Get winners
            winners = self.database_service.client.table('penny_signal_performance').select(
                '*'
            ).eq('status', 'CLOSED').order('return_pct', desc=True).limit(limit).execute()
            
            # Get losers
            losers = self.database_service.client.table('penny_signal_performance').select(
                '*'
            ).eq('status', 'CLOSED').order('return_pct', desc=False).limit(limit).execute()
            
            return {
                'winners': winners.data or [],
                'losers': losers.data or []
            }
