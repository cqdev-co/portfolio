...
class TickerService:
    ...
    def get_all_symbols(self, limit: Optional[int] = None) -> List[str]:
        ...
        try:
            ...
            if limit:
                response = self.client.table('penny_tickers').select(
                    'symbol'
    ...
    def get_symbols_by_exchange(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(all_symbols)} symbols from {exchange}"
            )
            ...
            raise DatabaseError(
                f"Failed to fetch symbols from {exchange}: {e}"
            )
    ...
    def get_symbols_by_sector(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(all_symbols)} symbols from {sector} sector"
            )
            ...
            raise DatabaseError(
                f"Failed to fetch symbols from {sector}: {e}"
            )
    ...
    def search_symbols(
        ...
        try:
            ...
            response = self.client.table('penny_tickers').select(
                ...
                f'symbol.ilike.{search_pattern},'
                f'name.ilike.{search_pattern}'
            ).limit(limit).execute()
            
            symbols = [row['symbol'] for row in response.data]
            logger.info(
                f"Found {len(symbols)} symbols matching '{search_term}'"
            )
    ...
    def get_available_exchanges(self) -> List[str]:
        ...
        try:
            ...
            exchanges = list(set(
                row['exchange'] for row in response.data 
                if row.get('exchange')
            ))
            exchanges.sort()
    ...
    def get_available_sectors(self) -> List[str]:
        ...
        try:
            ...
            sectors = list(set(
                row['sector'] for row in response.data 
                if row.get('sector')
            ))
            sectors.sort()
