...
class ExplosionSignal(BaseModel):
    ...
    volume_acceleration_2d: float = Field(
        description="2-day volume growth rate"
    )
    volume_acceleration_5d: float = Field(
        description="5-day volume growth rate"
    )
    ...
    consecutive_green_days: int = Field(
        description="Number of consecutive up days"
    )
    
    # Moving averages
    ema_20: float = Field(description="20-period EMA")
    ema_50: float = Field(description="50-period EMA")
    price_vs_ema20: float = Field(
        description="Price position vs EMA20 (%)"
    )
    price_vs_ema50: float = Field(
        description="Price position vs EMA50 (%)"
    )
    ema_crossover_signal: bool = Field(
    ...
    breaking_resistance: bool = Field(
        description="Breaking above key resistance level"
    )
    ...
    avg_spread_5d: Optional[float] = Field(
        None, description="5-day average spread %"
    )
    float_shares: Optional[int] = Field(
        None, description="Float shares"
    )
    is_low_float: bool = Field(
        default=False, description="Float < 50M shares"
    )
    daily_volatility: float = Field(
        description="Daily price volatility (ATR-based)"
    )
    atr_20: float = Field(description="20-period ATR")
    pump_dump_risk: RiskLevel = Field(
...
class AnalysisResult(BaseModel):
    ...
    # Main signal
    explosion_signal: ExplosionSignal = Field(
        description="Explosion setup signal"
    )
    
    # Optional AI analysis
    ai_analysis: Optional[AIAnalysis] = Field(
        None, description="AI-powered analysis"
    )
    
    # Overall assessment
    overall_score: float = Field(
        description="Overall signal score (0-1.0)"
    )
    opportunity_rank: OpportunityRank = Field(
        description="Opportunity tier ranking"
    )
    ...
    analysis_duration_ms: int = Field(
        description="Analysis execution time (ms)"
    )
    data_quality_score: float = Field(
        description="Input data quality score (0-1)"
    )
    
    def is_actionable(self) -> bool:
        """Check if signal meets criteria for action."""
        return (
            self.overall_score >= 0.60 and
            self.explosion_signal.dollar_volume >= 100000 and
            self.explosion_signal.volume_ratio >= 1.5 and
