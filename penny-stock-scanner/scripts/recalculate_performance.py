...
async def recalculate_with_stop_losses():
    ...
    response = (
        client.table("penny_signal_performance")
        .select("*")
        .eq("status", "CLOSED")
        .execute()
    )
    ...
    for i, trade in enumerate(trades, 1):
        ...
        try:
            ...
            result = await stop_checker.check_stop_loss_hit(
                symbol, entry_date, exit_date, entry_price, stop_loss_price
            )
            ...
            if result["stop_hit"]:
                ...
                print(
                    f"  🛑 STOP HIT! New exit: ${new_exit_price:.2f} on {new_exit_date}"
                )
                print(
                    f"     Old return: {trade['return_pct']:.2f}% → New return: {return_pct:.2f}%"
                )
                ...
                client.table("penny_signal_performance").update(update_data).eq(
                    "id", trade["id"]
                ).execute()
