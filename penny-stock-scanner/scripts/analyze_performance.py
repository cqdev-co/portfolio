...
def get_client():
    ...
    if not SUPABASE_URL or not SUPABASE_KEY:
        console.print("[red]Error: Supabase credentials not found in environment variables[/red]")
        console.print("Looking for: SUPABASE_URL or NEXT_PUBLIC_SUPABASE_URL")
        console.print("Looking for: SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY")
        return None
...
def analyze_signals():
    ...
    # Get all signals
    try:
        response = client.table('penny_stock_signals').select('*').order('scan_date', desc=True).execute()
    ...
    # === Basic Stats ===
    console.print(Panel(f"Total Signals in Database: [bold]{len(signals)}[/bold]", title="📈 Signal Volume"))
    ...
    for d in recent_dates:
        ...
        avg_score = sum(s['overall_score'] for s in day_signals) / len(day_signals) if day_signals else 0
    ...
    for rank in ['S', 'A', 'B', 'C', 'D']:
        if rank in rank_counts:
            ...
            avg = sum(rank_scores[rank]) / len(rank_scores[rank]) if rank_scores[rank] else 0
            rank_table.add_row(
                f"{rank}-Tier",
                str(rank_counts[rank]),
                f"{pct:.1f}%",
                f"{avg:.3f}"
            )
    ...
    setup_table.add_row("Breakouts 🚀", str(breakout_count), f"{breakout_count/len(signals)*100:.1f}%")
    setup_table.add_row("Consolidating 📦", str(consolidating_count), f"{consolidating_count/len(signals)*100:.1f}%")
    setup_table.add_row("Higher Lows 📈", str(higher_lows_count), f"{higher_lows_count/len(signals)*100:.1f}%")
    
    console.print(setup_table)
    
    # === Volume Analysis ===
    volume_ratios = [s.get('volume_ratio', 0) for s in signals if s.get('volume_ratio')]
    volume_spikes = [s.get('volume_spike_factor', 0) for s in signals if s.get('volume_spike_factor')]
    
    if volume_ratios:
        console.print("\n[bold]📊 Volume Analysis:[/bold]")
        console.print(f"  Avg Volume Ratio: {sum(volume_ratios)/len(volume_ratios):.2f}x")
        console.print(f"  Max Volume Ratio: {max(volume_ratios):.2f}x")
        console.print(f"  Signals with 2x+ Volume: {sum(1 for v in volume_ratios if v >= 2)}")
        console.print(f"  Signals with 3x+ Volume: {sum(1 for v in volume_ratios if v >= 3)}")
        console.print(f"  Signals with 5x+ Volume: {sum(1 for v in volume_ratios if v >= 5)}")
    ...
    if recent_high_quality:
        ...
        recent_table = Table(title="⭐ Recent High-Quality Signals (Last 7 Days, Score ≥ 0.75)")
...
def analyze_performance():
    ...
    if closed:
        ...
        # Returns
        returns = [p.get('return_pct', 0) for p in closed if p.get('return_pct') is not None]
        avg_return = sum(returns) / len(returns) if returns else 0
        
        winner_returns = [p.get('return_pct', 0) for p in winners if p.get('return_pct') is not None]
        loser_returns = [p.get('return_pct', 0) for p in losers if p.get('return_pct') is not None]
        ...
        # Days held
        days_held = [p.get('days_held', 0) for p in closed if p.get('days_held') is not None]
        if days_held:
        ...
        if returns:
            sorted_by_return = sorted(closed, key=lambda x: x.get('return_pct', 0) or 0, reverse=True)
...
def main():
    ...
    console.print("  • Watch for symbols that appear frequently - they may have persistent setups")
