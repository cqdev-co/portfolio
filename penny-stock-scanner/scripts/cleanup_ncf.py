...
async def cleanup_ncf_to():
    ...
    response = (
        client.table("penny_signal_performance").delete().eq("symbol", symbol).execute()
    )
    ...
    response = (
        client.table("penny_stock_signals").delete().eq("symbol", symbol).execute()
    )
