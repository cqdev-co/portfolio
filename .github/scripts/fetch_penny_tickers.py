...
class PennyTickerFetcher:
    ...
    def fetch_fmp_penny_tickers(self) -> List[PennyTickerInfo]:
        ...
        try:
            ...
            self.logger.info(
                "Fetching penny tickers from Financial Modeling Prep..."
            )
            ...
            for item in data:
                symbol = (
                    ...
                    if item.get('symbol') else ''
                )
                name = (
                    item.get('companyName', '').strip() 
                    if item.get('companyName') else ''
                )
    ...
    def fetch_alpha_vantage_penny_tickers(self) -> List[PennyTickerInfo]:
        ...
        try:
            ...
            if (
                ...
                self.logger.warning(
                    f"Alpha Vantage returned empty/invalid response"
                )
                return []
            
            if response_text.startswith('{') and response_text.endswith('}'):
                self.logger.warning(
                    "Alpha Vantage returned JSON error response"
                )
                return []
    ...
    def deduplicate_tickers(
        self, 
        all_tickers: List[PennyTickerInfo]
        ...
        try:
            ...
            for ticker in tickers:
                ...
                if result and result.is_valid:
                    ...
                    if (
                        result.last_price and 
                        self.penny_filter.min_price <= 
                        result.last_price <= 
    ...
    def run(self) -> bool:
        ...
        if not validated_tickers:
            self.logger.error(
                "No penny tickers remaining after YFinance validation"
            )
            return False
