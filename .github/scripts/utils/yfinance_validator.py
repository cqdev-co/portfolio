...
class YFinanceValidator:
    ...
    def validate_ticker(
        self, symbol: str, period: str = "6mo", max_retries: int = 3
    ) -> YFinanceValidation:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                days_since_last = (datetime.now() - last_date_naive).days
    ...
    def _validate_ohlc_quality(self, hist: pd.DataFrame) -> Tuple[float, List[str]]:
        ...
        if valid_ratio < 0.95:  # Less than 95% valid OHLC
            issues.append(f"OHLC violations in {(1 - valid_ratio) * 100:.1f}% of data")
    ...
    def validate_batch(
        ...
        logger.warning("Note: Validation may take 10-20 minutes due to rate limiting")
        ...
        for batch_num, batch_start in enumerate(range(0, len(symbols), batch_size)):
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                future_to_symbol = {
                    executor.submit(self.validate_ticker, symbol, period): symbol
    ...
    def get_validation_summary(self, results: Dict[str, YFinanceValidation]) -> Dict:
        ...
        summary = {
            ...
            "avg_ohlc_quality": np.mean([r.ohlc_quality for r in valid_results])
            if valid_results
            else 0,
            "avg_history_days": np.mean([r.min_history_days for r in valid_results])
            if valid_results
            else 0,
            "avg_volume": np.mean([r.avg_daily_volume for r in valid_results])
            if valid_results
            else 0,
            "price_range": {
                "min": min(
                    [r.last_price for r in valid_results if r.last_price], default=0
                ),
                "max": max(
                    [r.last_price for r in valid_results if r.last_price], default=0
                ),
                "avg": np.mean([r.last_price for r in valid_results if r.last_price])
                if valid_results
                else 0,
            },
            "top_rejection_reasons": [
                {"reason": reason, "count": count} for reason, count in top_issues
            ],
            "has_recent_data_count": sum(1 for r in valid_results if r.has_recent_data),
            "has_price_data_count": sum(1 for r in valid_results if r.has_price_data),
            "has_volume_count": sum(1 for r in valid_results if r.has_volume),
            "has_info_count": sum(1 for r in valid_results if r.has_info),
        }
