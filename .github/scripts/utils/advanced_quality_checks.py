...
class AdvancedQualityChecker:
    ...
    def check_ticker(
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    if inst_holders is not None and not inst_holders.empty:
                        ...
                        metrics.institutional_holders = (
                            10 if metrics.institutional_ownership > 0.3 else
                            5 if metrics.institutional_ownership > 0.1 else 0
                        )
                except Exception:
                ...
                metrics.net_income = info.get('netIncomeToCommon') or \
                    info.get('netIncome')
                ...
                metrics.passes_fundamental_check = (
                    metrics.has_positive_revenue and
                    (metrics.revenue or 0) >= self.min_revenue
                )
                
                if not metrics.passes_fundamental_check:
                    rev_str = (
                        f"${metrics.revenue/1e6:.1f}M" 
                        if metrics.revenue else "N/A"
                    )
                    metrics.quality_issues.append(
                ...
                metrics.passes_float_check = (
                    metrics.float_shares is not None and
                    metrics.float_shares >= self.min_float_shares and
                    metrics.float_percent >= self.min_float_percent and
                    metrics.short_percent <= self.max_short_percent
                )
                
                if not metrics.passes_float_check:
                    float_str = (
                        f"{metrics.float_shares/1e6:.1f}M" 
                        if metrics.float_shares else "N/A"
                    )
                    metrics.quality_issues.append(
    ...
    def check_batch(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_symbol = {
                executor.submit(self.check_ticker, symbol): symbol
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    results[symbol] = AdvancedQualityMetrics(
                        symbol=symbol,
                        quality_issues=[f"Check failed: {e}"]
                    )
        ...
        avg_score = sum(
            r.advanced_quality_score for r in results.values()
    ...
    def get_summary(
        self, 
        results: Dict[str, AdvancedQualityMetrics]
    ) -> Dict:
        ...
        return {
            ...
            'passes_options': sum(
                1 for m in metrics_list if m.passes_options_check
            ),
            ...
            'passes_float': sum(
                1 for m in metrics_list if m.passes_float_check
            ),
            ...
            'avg_advanced_score': sum(
                m.advanced_quality_score for m in metrics_list
            ) / len(metrics_list),
