...
def store_tickers(
    ...
    try:
        ...
        for ticker in tickers:
            ...
            data = {
                ...
                'exchange': (
                    ...
                    if getattr(ticker, 'exchange', None) else None
                ),
                'country': (
                    ticker.country.strip() 
                    if getattr(ticker, 'country', None) else None
                ),
                'currency': (
                    ticker.currency.strip() 
                    if getattr(ticker, 'currency', None) else None
                ),
                'sector': (
                    ticker.sector.strip() 
                    if getattr(ticker, 'sector', None) else None
                ),
                'industry': (
                    ticker.industry.strip() 
                    if getattr(ticker, 'industry', None) else None
                ),
        ...
        logger.info(
            f"Successfully stored {total_inserted} tickers in {table_name}"
        )
