...
from .advanced_quality_checks import AdvancedQualityChecker
...
class EfficientTickerFilter:
    ...
    def __init__(
        ...
        self.us_exchanges = {"NASDAQ", "NYSE", "NYSEARCA", "NYSEMKT"}

        # Major exchanges only (most liquid and reliable)
        self.major_exchanges = {"NASDAQ", "NYSE"}
        ...
        self.quality_indexed_symbols = (
            self.sp500_symbols | self.nasdaq100_symbols | self.dow30_symbols
        )
    ...
    def pre_filter_tickers_fast(self, tickers: list[dict]) -> list[dict]:
        ...
        for ticker in tickers:
            ...
            if any(
                ...
                for pattern in ["TEST", "TEMP", "NULL", "VOID"]
            ):
                symbol_filtered += 1
                continue
            ...
            is_major_exchange = exchange and any(
                major_ex in exchange.upper() for major_ex in self.major_exchanges
            )
            is_us_exchange = exchange and any(
                us_ex in exchange.upper() for us_ex in self.us_exchanges
            )
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            try:
                ...
                for item in data:
                    ...
                    is_valid = (
                        market_cap
                        and market_cap >= self.min_market_cap
                        and price
                        and self.min_price <= price <= self.max_price
                        and volume
                        and volume >= self.min_volume
                        and dollar_volume
                        >= self.min_dollar_volume  # NEW: dollar volume
                    )
                    ...
                    if is_valid:
                        ...
                        if not market_cap or market_cap < self.min_market_cap:
                            ...
                            rejection_reason = (
                                f"Volume {volume:,.0f} < {self.min_volume:,.0f}"
                            )
    ...
    def _batch_yfinance_metrics(
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            try:
                ...
                for symbol in batch_symbols:
                    try:
                        ...
                        price = (
                            ...
                            if "Close" in recent_data
                            else None
                        )
                        volume = (
                            recent_data["Volume"].mean()
                            if "Volume" in recent_data
                            else None
                        )
                        ...
                        is_valid = (
                            ...
                            and volume
                            and volume >= self.min_volume
                            and dollar_volume >= self.min_dollar_volume
                        )
                ...
                logger.warning(
                    f"Error fetching yfinance batch {i // batch_size + 1}: {e}"
                )
                continue
    ...
    def filter_high_quality_tickers(
        ...
        if len(pre_filtered) > hard_limit:
            logger.info(
                f"HARD LIMIT: Reducing from {len(pre_filtered)} to {hard_limit} tickers for system performance"
            )
    ...
    def get_filtering_summary(
        ...
        summary = {
            ...
            "price_range": {
                ...
                "avg": np.mean([m.price for m in valid_metrics if m.price])
                if valid_metrics
                else 0,
            },
            "volume_stats": {
                ...
                "avg": np.mean([m.volume for m in valid_metrics if m.volume])
                if valid_metrics
                else 0,
            },
        }
    ...
    def filter_with_advanced_checks(
        ...
        basic_symbols, basic_metrics = self.filter_high_quality_tickers(
            ...
            - 20,  # Lower threshold, advanced adds points
        ...
        if enable_advanced_checks and len(basic_symbols) > 0:
            ...
            basic_symbols, metrics_dict = self.apply_advanced_quality_checks(
                basic_symbols, metrics_dict, verbose=verbose
            )
        else:
        ...
        # Summary statistics
        avg_score = (
            np.mean([m.quality_score for m in final_metrics]) if final_metrics else 0
        )
