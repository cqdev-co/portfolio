...
def is_cfd_ticker(symbol: str, name: str, exchange: str = "") -> bool:
    ...
    cfd_symbol_patterns = [
        ...
        "cfd",
        ".c",
        "_c",  # Direct CFD indicators
        ".d",
        "_d",  # Derivative indicators
        ".x",
        "_x",  # Some brokers use X suffix for CFDs
    ]
    ...
    cfd_exchange_patterns = ["cfd", "derivative", "synthetic", "swap"]
    ...
    if any(
        ...
        if any(
            cfd_indicator in name_lower
            for cfd_indicator in [
                "cfd",
                "contract",
                "derivative",
                "synthetic",
    ...
    if any(pattern in symbol_lower for pattern in ["cfd", "der", "syn", "lev", "spr"]):
        return True
